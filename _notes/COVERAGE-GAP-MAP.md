...
| Frees plot and crew; refunds no water | Lifecycle | BUILT | `KingdomSocket.OnCleared` `Growth/KingdomSocket.cs:337`; no-refund stated `Growth/KingdomMaterials*.cs` | — |
...
| Protection law: nothing player-placed is ever cleared | Bindings | BUILT | explicit contract at `Growth/KingdomPlot2.02.KingdomPlotQuote.cs:16`; plot sweep scoped to `PlotId` `Growth/KingdomSocket.cs:378` | — |
...
| Add. 7 — bits price high-craft builds and certified-tech repair | BUILT | Read off vanilla `TinkerItem` `Growth/KingdomMaterials*.cs`; spent `:711-778`; **repair path real** `Growth/KingdomWear.cs:593-602` | Content is two entries (`waterworks`, `chargingpost`) |
