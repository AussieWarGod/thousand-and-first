...
| **…a chronicled *ceremony*** | Lifecycle | **PARTIAL** | Chronicled twice — condemnation `Growth/KingdomMaterials*.cs`, completion `:1862` | No ceremony hook: no attendance roll, no shared water, no witnesses — unlike the raising, which has all three (`Experience/KingdomCeremony.cs:96-131`) |
...
| Notable tastes | PARTIAL | Ten categories `Experience/KingdomCeremonyRules.Tastes.cs:22` (`TasteCategories`); deterministic draw, matching, chronicle, and complaint prose remain in that shard | **"Met tastes shade equilibrium up" is not wired.** `Experience/KingdomCeremony.cs:208-209` computes `shade` and writes it **only to the debug log**. `Equilibrium(Water, Food, Roof, Lift)` (`Growth/KingdomCatalogueRules*.cs`) has no shade argument and its one production caller passes only summed `Carries` |
| Leader traits | PARTIAL | One virtue + one flaw, deterministic, no reroll `Experience/KingdomCeremonyRules.LeaderAndShade.cs:58` (`ChooseLeaderTraits`); chronicled in the same shard; **do real work** in bounty decisions `Quests/KingdomBountyRules*.cs` | Same discard: `LeaderShade()`'s sole non-test call site is a log line at `Experience/KingdomCeremony.cs:197` |
