...
- **Upkeep.** `KingdomRules.UpkeepDrams(Population, Stage)` = `Population × StageUpkeepPercent / 100`,
  with `StageUpkeepPercent = {100, 120, 150, 180, 220}` by stage (`Core/KingdomRules*.cs`).
  One dram per settler per day at camp; 2.2 at city.
- **Reserve.** `ReserveDays = 3` (`Core/KingdomRules*.cs`) is a *quantity*, not a clock cap — the
...
- **Fetch.** `FetchableDrams(Hands, OpenWater, StorageSpace, Days)` = `Hands × 2 × Days`, clamped by
  ...
  (`Core/KingdomRules*.cs`; `FetchDramsPerSettler = 2` at `:441`). Hands are **named**: only
  settlers the founder put on the water detail walk to the water
  (`Core/KingdomCharterPart*.cs`). An empty detail means the settlement drinks only what the
  founder pours in.
