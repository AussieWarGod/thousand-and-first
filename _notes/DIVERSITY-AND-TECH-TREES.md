...
|---|---|---|---|---|
| **City `Style`** | `common`, `verdant`, `fungal`, `gyre`, `eater` (`T/KingdomBuildings.xml:83-87`) | `KingdomRules.StyleForSite(blueprint, region, zLevel)` at founding (`T/Core/KingdomFounding.cs:60,181,259`) | design eligibility (`KingdomRules.StyleAllows`, `T/Growth/KingdomCommission.cs:48`, `Growth/KingdomSocket*.cs`), upgrade eligibility (`KingdomUpgrade*.cs`), wall material (`Growth/KingdomMaterialRules*.cs`), wall blueprint (`Growth/KingdomPlotRules*.cs`), crop species and crop days (`KingdomCropRules.cs:305,337,400`) | **NO.** All 63 designs are `Styles="all"`. Skins vary; designs do not. |
