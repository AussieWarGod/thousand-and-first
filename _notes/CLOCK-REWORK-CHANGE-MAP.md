...
`KingdomRules.StageFor` (`Core/KingdomRules.cs:942`) is a pure population + liquid-storage-capacity
...
| **`Withered`** — `System.Withered`, set at `Growth/KingdomGrowth.cs:192`, halves sealed vigour (`Core/KingdomRules.cs:1064`). | derived from `DryStreak` | Reconcile with subsidence: withering is *level loss*, so it should read the same fixed point rather than a parallel flag. | **B** | Medium — duplicate concept. |
...
| **Road rotation** — `RotationStart(TimeTicks, Count)` = `(ticks / TicksPerDay) % Count` (`Growth/KingdomRoadRules.cs:304`). | RAW, index only | Already correct — a scheduling index, stable across reloads. | **A** | Low. |
...
| System / counter | Today's denominator | Doctrine denominator | Class | Risk |
|---|---|---|---|---|
| **Plot stage ratchet** — `StageAt(Elapsed, Total)` (`Growth/KingdomPlotRules.cs:1469`), driven from `r_KingdomPlotWorks.StartTick`/`TotalTicks` (`Growth/KingdomPlot2.cs:1563`). Uncapped, applies every crossed stage in order. | RAW | Already the doctrine's shape — a hundred days finds it finished, one finds it framed. | **A** | Low. |
...
| **Upgrade reserve** — `ReserveDrams` = `UpkeepDrams(Pop, Stage) × MaxUpkeepDaysCharged` (`Growth/KingdomUpgradeRules.cs:254`). | cap-as-quantity | Same. | **D**-dependency | Medium. |
...
| **Ceremony `IsAttended`** — `NowTicks - CompleteTick < TicksPerDay` (`Experience/KingdomCeremonyRules.cs:126`). | RAW classifier | **The exemplar of awareness-crystallisation done right**: it changes prose, never outcome. Model the brink's announcement on it. | **A** | Low. |
...
| System / counter | Today's denominator | Doctrine denominator | Class | Risk |
|---|---|---|---|---|
| **Manifest deadline** — `ManifestWindowDays = 10`, `DeadlineTick`, `ManifestExpired` strictly-past (`Trade/KingdomManifest.cs:88,118,127`). Two-strike `ExpireManifestIfStale` (`Trade/KingdomTrade.cs:161-199`): first expiry turns the load back and **re-stamps a fresh deadline at the moment of witnessing**; second sets the water down as a real pool. Never a debt. | RAW, re-stamped on observation | **Already brink-shaped and already correct.** Fold its re-stamp into the shared helper. | **A** / **B** (consolidate) | Low. The turn-back *path* is untested. |
...
| **Bounty due-ticks** — `TakenTick`/`DueTick`, `WorkDays` (`Quests/KingdomBounty.cs:615-617`; `Quests/KingdomBountyRules.cs:629`). Haul 1–5 d, Manning 30 d, Scouting 4 d, **Clearance 0 (no clock — read off the world)**. No expiry, ever. Class doc `:18-21` is a doctrine statement in its own right. | RAW due-ticks | Correct. | **A** | Low. |
| **Bounty manning** — `ManOneWork` runs **every attended pass** while `now < DueTick`, then the due-tick ends the season (`Quests/KingdomBounty.cs:641-649`). | **PASS for the labour, RAW for the finish** — mixed denominators inside one case | Labour becomes time × hands like every other work. | **B** | Low. |
...
| **Carry-sign haul** — `HaulDueTick` (`Experience/KingdomGuestRules.cs:346`), no expiry whatsoever. | RAW | Correct. | **A** | Low. |
| **Deed memory** — `DeedMemoryTicks = 12000` (`Core/KingdomRules.cs:846`), flavour text only. | RAW | Correct. | **A** | Low. |
...
does not build a second one"* (`Experience/KingdomWaterRiteRules.cs:494-497`).
...
version + `WriteNamedFields`, `Core/KingdomSystem.cs:805-865`). Unknown names are skipped, missing
...
| ~40 code doc comments | The canonical phrasings are `Core/KingdomRules.cs:290-291`, `Growth/KingdomPowerRules.cs:241-243`, `Growth/KingdomRoadRules.cs:21-25` and `:247-249`, `Growth/KingdomWear.cs:92-94`, `Growth/KingdomWearRules.cs:8-12`, `Growth/KingdomMaterials.cs:1677-1681`, `Growth/KingdomMaterialRules.cs:1760-1764`, `Core/KingdomConversionRules.cs:112-120`, `Growth/KingdomLodgingRules.cs:686-690`. |
