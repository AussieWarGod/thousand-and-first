...
## [0.3.6] — 2026-09-13 (Alpha candidate preparation)

Corrected release candidate for immediate claimed-zone lighting and full-zone city sight.
...
actual Alpha metadata before tagging. The proposed native .NET source-test route retains both
full licensed suites, zero skips, Windows engine compilation and native behavioral gates.
Current 0.3.6 delivery and native validation are pending. Earlier private 0.3.5 evidence retains
its original source and does not become a new-version PASS.
...
- Corrected Quickstart's post-placement ground check to admit only the exact founder, and its
  ...
  construction. The new heart check avoids display-name callbacks. Actual boot tests exposed
...
- A thrown raid notification callback now retains a quarantined operation instead of silently
  becoming a lost delivery on recovery. Quarantine precedes exception text and diagnostics; even
  a broken logger cannot make an uncertain delivery retire as successful. Existing successful
  delivery and explicit-false recovery behavior are unchanged.
- Developer persona runs preserve prior profiles and refuse pre-existing games. Shutdown and
  screenshots require an exact launch-ownership receipt; global game kills and profile wipes are
  removed. Native process and real-shell lifecycle regressions cover the replacement.
...
- **Addendum 9 line-cap debt is cleared; semantic review remains an executable release blocker.**
  ...
  boundaries, bound to the exact source digest. The hardening sequence adds 144 semantic
  decompositions to the prior ten, for 154 oversized authorities split without changing public or
  serialized type identity. Twenty-five post-`2cb97fc` decompositions use cohesive declaration/protocol shards
  ...
  city reconciliation;
  thirteen later families split central logistics, residents, physical happenings, porters,
  purpose cargo, trade state, city-book columns, founding, raid incidents, zoning, creed, crops,
  ...
  since checkpoint `b049c17`, and thirteen since hosted checkpoint `1c2d619`. Current 2945-file census is line-cap green: 420,767 physical lines; 0 files exceed 300,
