...
namespace ThousandAndFirst
{
	public static partial class KingdomCrops
	{
		...
		public static void AttemptSow(GameObject Actor, GameObject Seed)
		{
			...
			string seedFailure;
			// The window opens BEFORE the commit and closes in the finally below. Every path from
			// here on may still roll this receipt back, and the rows laid and the seed destroyed in
			// between are real engine callbacks. Rollback re-proves each bound vessel's MaxVolume,
			// so a basin widening landing in that span would refuse the compensation for good.
			...
			{
				// Closed on EVERY exit, sown or refused: the hold must not outlive this sowing.
