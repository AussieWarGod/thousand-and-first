...
namespace ThousandAndFirst
{
	internal static class KingdomResidentDeathCodec
	{
		...
		private const int ResidenceMagic = 0x32445254;
		...
		internal static bool TryEncode(KingdomResidentDeathJournal journal, out string wire)
		{
			...
			// Keep old canonical bytes while every before-row has unknown residence. A mixed
			// journal uses rd2 for every row, so its appended field cannot shift the next receipt.
			bool residence = false;
			foreach (var receipt in journal.Entries)
				residence |= !string.IsNullOrEmpty(receipt.Before.Residence);
			try
			{
				using (var stream = new MemoryStream())
				using (var w = new BinaryWriter(stream, Utf8, true))
				{
					w.Write(residence ? ResidenceMagic : Magic); w.Write(journal.Realm); w.Write(journal.Settlement); w.Write(journal.Entries.Count);
					foreach (var r in journal.Entries) Write(w, r, residence);
					w.Flush();
					if (stream.Length > MaxWire / 4 * 3 - 4) return false;
					wire = (residence ? "rd2:" : "rd1:") + Convert.ToBase64String(stream.ToArray()); return wire.Length <= MaxWire;
				}
			}
		...
		{
			journal = null;
			if (wire == null || wire.Length > MaxWire) return false;
			bool residence = wire.StartsWith("rd2:", StringComparison.Ordinal);
			if (!residence && !wire.StartsWith("rd1:", StringComparison.Ordinal)) return false;
			try
			{
				...
				{
					if (r.ReadInt32() != (residence ? ResidenceMagic : Magic)) return false;
					...
					for (int i = 0; i < count; i++) rows.Add(Read(r, residence));
		...
		internal static string Row(KingdomResidentRow row)
		{
			...
			{ WriteRow(w, row, !string.IsNullOrEmpty(row.Residence)); w.Flush(); return Convert.ToBase64String(s.ToArray()); }
		}
		...
		private static void Write(BinaryWriter w, KingdomResidentDeathReceipt r, bool residence)
		{
			w.Write(r.Realm); w.Write(r.Settlement); w.Write(r.SettlementName); w.Write(r.Body); w.Write(r.Zone);
			w.Write(r.Tick); w.Write(r.MintedTick); WriteRow(w, r.Before, residence); w.Write((byte)r.Cause); w.Write(r.Memory);
		...
		private static KingdomResidentDeathReceipt Read(BinaryReader r, bool residence)
		{
			var v = new KingdomResidentDeathReceipt
			{
				...
				Tick = r.ReadInt64(), MintedTick = r.ReadInt64(), Before = ReadRow(r, residence), Cause = (KingdomStandingCause)r.ReadByte(), Memory = Flag(r),
		...
		private static void WriteRow(BinaryWriter w, KingdomResidentRow r, bool residence)
		{
			...
			if (residence) w.Write(r.Residence ?? "");
		}
		private static KingdomResidentRow ReadRow(BinaryReader r, bool residence)
		{
			...
			return new KingdomResidentRow(id, name, originCode, creedCode, tick, home, job, role, day, standing, cause, zone,
				roof, creed, toward, channel, Nullable(r), origin, arrived, residence ? r.ReadString() : "");
		}
