...
namespace ThousandAndFirst.Tests
{
	...
	public sealed class KingdomRemembranceRulesTests
	{
		...
		public void ProjectionIsExactlyOnceCodecStableAndLossCannotDuplicate()
		{
			...
			ClassicAssert.IsTrue(Prepare(ledger, "one", 7, "Ari", 8, "Ula", "cairn-1", 20));
			ClassicAssert.IsTrue(KingdomExperienceRules.TryGetRemembrance(ledger,
				"taf:settlement:one", out KingdomRemembranceReceipt prepared,
				out string failure), failure);
			ClassicAssert.AreEqual(KingdomRemembrancePhase.ProjectionPrepared, prepared.Phase);
			byte[] once = Bytes(ledger);
			ClassicAssert.IsTrue(KingdomExperienceRules.TryPrepareRemembranceProjection(ledger,
			...
			ClassicAssert.IsTrue(KingdomExperienceRules.TryCompleteRemembranceProjection(ledger,
			...
			ClassicAssert.IsTrue(KingdomExperienceRules.TryGetRemembrance(loaded,
				prepared.SettlementId, out KingdomRemembranceReceipt projected, out failure), failure);
			ClassicAssert.AreEqual(KingdomRemembrancePhase.Projected, projected.Phase);
			ClassicAssert.IsTrue(KingdomExperienceRules.TryMarkRemembranceLost(loaded,
				loaded.Revision, projected.SettlementId, projected.CarrierObjectId,
				out failure), failure);
			ClassicAssert.AreEqual(KingdomRemembrancePhase.Lost, loaded.Remembrances[0].Phase);
			byte[] lost = Bytes(loaded);
			ClassicAssert.IsFalse(Prepare(loaded, "one", 9, "Other", 8, "Ula", "cairn-2", 30));
		...
		public void PreparedCarrierLossRetainsPermanentSemanticRow()
		{
			KingdomExperienceLedger ledger = Bound();
			ClassicAssert.IsTrue(Prepare(ledger, "one", 7, "Ari", 8, "Ula", "cairn-1", 20));
			ClassicAssert.IsTrue(KingdomExperienceRules.TryMarkRemembranceLost(ledger,
				ledger.Revision, "taf:settlement:one", "cairn-1", out string failure), failure);
			ClassicAssert.AreEqual(KingdomRemembrancePhase.Lost, ledger.Remembrances[0].Phase);
			ClassicAssert.AreEqual(7, ledger.Remembrances[0].SubjectResidentId);
			ClassicAssert.AreEqual("Ari", ledger.Remembrances[0].SubjectName);
		}
		...
		{
			KingdomExperienceLedger ledger = Bound();
			ClassicAssert.IsTrue(Eligible(ledger, "one", 7, "Ari", 20));
			long beforeRevision = ledger.Revision;
			ClassicAssert.IsTrue(KingdomExperienceRules.TryDeclineRemembrance(ledger, ledger.Revision,
				"taf:settlement:one", "City one", 7, "Ari", 8, "Ula", 20,
				out string failure), failure);
			ClassicAssert.AreEqual(beforeRevision + 1, ledger.Revision);
			ClassicAssert.AreEqual(KingdomRemembrancePhase.Declined, ledger.Remembrances[0].Phase);
			byte[] declined = Bytes(ledger);
			ClassicAssert.IsTrue(KingdomExperienceRules.TryDeclineRemembrance(ledger,
				beforeRevision, "taf:settlement:one", "City one", 7, "Ari", 8, "Ula", 99,
				out failure), failure);
			CollectionAssert.AreEqual(declined, Bytes(ledger));
			ClassicAssert.IsFalse(KingdomExperienceRules.TryDeclineRemembrance(ledger,
		...
		{
			KingdomExperienceLedger ledger = Bound();
			ClassicAssert.IsTrue(Eligible(ledger, "one", 1, "A", 10));
			byte[] eligible = Bytes(ledger);
			ClassicAssert.IsFalse(KingdomExperienceRules.TryPrepareRemembranceProjection(ledger,
			...
			for (int i = 1; i < KingdomExperienceRules.MaxRemembranceReceipts; i++)
				ClassicAssert.IsTrue(Prepare(ledger, "city" + i, i + 1, "Dead" + i, i + 11,
					"Mourner" + i, "cairn-" + i, 20 + i));
			byte[] atCap = Bytes(ledger);
			ClassicAssert.IsFalse(Prepare(ledger, "city3", 4, "Dead3", 14, "Mourner3",
				"cairn-3", 24));
		...
		public void MaximumCivicRowsStayInsideApprovedSixteenKibEnvelope()
		{
			...
			for (int i = 0; i < 3; i++)
			{
				...
				string zone = new string((char)('u' + i),
					KingdomExperienceRules.MaxCivicTextBytes);
				ClassicAssert.IsTrue(KingdomExperienceRules.TryPrepareOfficeAppointment(ledger,
				...
				ClassicAssert.IsTrue(KingdomExperienceRules.TryCreateRemembranceEligibility(ledger,
			...
			ClassicAssert.LessOrEqual(envelope.Length, KingdomExperienceCodec.MaxEnvelopeBytes);
			ClassicAssert.LessOrEqual(envelope.Length,
				KingdomExperienceRules.MaxDeclaredPayloadBytes + 12);
			ClassicAssert.IsTrue(KingdomExperienceRules.TryValidate(ledger, out string valid), valid);
		}
		...
		public void OnlyExactDirectWitnessEligibilityCanBeUsedAndItNeverExpires()
		{
			...
			ClassicAssert.AreEqual(KingdomRemembrancePhase.Eligible, loaded.Remembrances[0].Phase);
			ClassicAssert.AreEqual(20L, loaded.Remembrances[0].DecidedTick);
			ClassicAssert.IsTrue(KingdomExperienceRules.TryCreateRemembranceEligibility(ledger,
				ledger.Revision - 1, "taf:settlement:one", "City one", 7, "Ari", 20,
				out failure), failure);
			CollectionAssert.AreEqual(once, Bytes(ledger));
			ClassicAssert.IsFalse(KingdomExperienceRules.TryCreateRemembranceEligibility(ledger,
			...
			ClassicAssert.AreEqual(20L, ledger.Remembrances[0].DecidedTick);
		}
		...
		public void CivicRowsCannotShareResidentSubjectOrProjectionIdentity()
		{
			...
			ClassicAssert.IsTrue(Eligible(ledger, "two", 8, "Dead", 11));
			...
			ClassicAssert.IsFalse(KingdomExperienceRules.TryValidate(duplicate, out failure));
