...
namespace ThousandAndFirst.Tests
{
	...
	public sealed class KingdomResidentDeathPublicationTests
	{
		...
		public void ExactWitnessPublishesOnlyStandingCauseDespiteFrozenRung(bool selected)
		{
			...
			ClassicAssert.IsTrue(KingdomSubsidenceRungRules.BlocksProjection(wire));
			ClassicAssert.IsFalse(city.TryPublish(Read(city), out _));
			ClassicAssert.IsTrue(city.TryPublishWitnessedDeath(death, wire, () => true));
			ClassicAssert.AreEqual(1, KingdomResidentDeathRules.RowCut(death, Resident(city)));
			ClassicAssert.AreEqual(wire, city.SubsidenceModel);
			ClassicAssert.AreSame(names, city.ResidentNames); ClassicAssert.AreSame(roofs, city.ResidentRoofStanding);
			ClassicAssert.AreEqual(selected, KingdomSubsidenceRungRules.BlocksRoof(wire, 11));
			ClassicAssert.IsTrue(KingdomSubsidenceRungRules.BlocksProjection(wire));
			ClassicAssert.IsTrue(city.TryPublishWitnessedDeath(death, wire, () => true));
			ClassicAssert.AreEqual(1, KingdomResidentDeathRules.RowCut(death, Resident(city)));
		}
		...
		public void ForeignOrUnprovedAuthorityDoesNotAcquireADeathCut(string change)
		{
			...
			ClassicAssert.IsFalse(city.TryPublishWitnessedDeath(death, wire, () => change != "missing-intent"));
			ClassicAssert.AreEqual(before, Shape(city)); ClassicAssert.AreEqual(wire, city.SubsidenceModel);
		}
		[Test]
		public void AuthorityCallbackChangingNameRefusesWithoutStandingWrite()
		{
			var city = Fixture(false, out var death);
			ClassicAssert.IsFalse(city.TryPublishWitnessedDeath(death, city.SubsidenceModel,
				() => { city.ResidentNames[0] = "Callback changed name"; return true; }));
			ClassicAssert.AreEqual((int)KingdomResidentStanding.Resident, city.ResidentStandings[0]);
		...
		public void LostPostWriteProofRetainsAfterCutForExactRetry()
		{
			...
			ClassicAssert.IsFalse(city.TryPublishWitnessedDeath(death, city.SubsidenceModel, () => ++checks == 1));
			ClassicAssert.AreEqual(1, KingdomResidentDeathRules.RowCut(death, Resident(city)));
			ClassicAssert.IsTrue(city.TryPublishWitnessedDeath(death, city.SubsidenceModel, () => true));
		}
		[Test]
		public void StandingWrittenReceiptRequiresItsOwnAfterCut()
		{
			...
			ClassicAssert.IsFalse(city.TryPublishWitnessedDeath(death, city.SubsidenceModel, () => true));
		...
		private static KingdomCityBook Fixture(bool selected, out KingdomResidentDeathReceipt death)
		{
			...
			var city = new KingdomCityBook(); ClassicAssert.IsTrue(city.TryPublish(state, out _));
			...
			ClassicAssert.IsTrue(KingdomSubsidenceStepRules.TryFreezeRungPlan(step, plan, out step));
			ClassicAssert.IsTrue(KingdomSubsidenceStepCodec.TryEncode(step, out string wire)); city.SubsidenceModel = wire;
			...
			ClassicAssert.IsTrue(KingdomResidentDeathRules.Valid(death)); return city;
		}
		private static KingdomCityState Read(KingdomCityBook city)
		{ ClassicAssert.IsTrue(city.TryReadExact(out var state, out _)); return state; }
		private static KingdomResidentRow Resident(KingdomCityBook city)
		{ var state = Read(city); ClassicAssert.IsTrue(state.TryResident(0, out var row)); return row; }
