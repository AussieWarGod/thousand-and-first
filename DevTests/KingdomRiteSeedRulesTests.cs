...
namespace ThousandAndFirst.Tests
{
	...
	public class KingdomRiteSeedRulesTests
	{
		...
		{
			ClassicAssert.IsTrue(KingdomResearchRules.MayRememberRite(true, "Barathrumites"));
			ClassicAssert.IsFalse(KingdomResearchRules.MayRememberRite(false, "Barathrumites"));
			ClassicAssert.IsFalse(KingdomResearchRules.MayRememberRite(true, null));
			ClassicAssert.IsFalse(KingdomResearchRules.MayRememberRite(true, "Fungi|Oozes"),
			...
			ClassicAssert.IsFalse(KingdomResearchRules.MayRememberRite(true,
				new string('x', KingdomResearchRules.MaxFounderRiteNameLength + 1)));
			ClassicAssert.IsFalse(KingdomResearchRules.IsCanonicalFounderRite("rite:"));
			ClassicAssert.IsFalse(KingdomResearchRules.IsCanonicalFounderRite("rite"));
			ClassicAssert.IsFalse(KingdomResearchRules.IsCanonicalFounderRite("rite:Fungi,Oozes"));
			ClassicAssert.IsTrue(KingdomResearchRules.IsCanonicalFounderRite(" RITE:Barathrumites "));
		...
		public void RuntimeHook_RegistersInitialVanillaRiteAndRoutesItThroughResearch()
		{
			...
			ClassicAssert.GreaterOrEqual(registration, 0);
			int registrationEnd = source.IndexOf("\n\t\t/// <summary>", registration + 1,
				StringComparison.Ordinal);
			ClassicAssert.Greater(registrationEnd, registration);
			...
			ClassicAssert.AreEqual(1, Occurrences(source, "Registrar.Register(WaterRitualStartEvent.ID)"),
			...
			int handler = source.IndexOf("public override bool HandleEvent(WaterRitualStartEvent E)",
				StringComparison.Ordinal);
			ClassicAssert.GreaterOrEqual(handler, 0);
			int handlerEnd = source.IndexOf("\n\t\t/// <summary>", handler + 1, StringComparison.Ordinal);
			ClassicAssert.Greater(handlerEnd, handler);
			...
			ClassicAssert.GreaterOrEqual(remember, 0);
			int rememberEnd = source.IndexOf("\n\t\t// The first token", remember, StringComparison.Ordinal);
			ClassicAssert.Greater(rememberEnd, remember);
		...
		public void ConcreteSourceResolution_DeduplicatesBareAndQualifiedAliases()
		{
			...
			ClassicAssert.AreEqual("rite:fungi", KingdomZoningRules.SatisfyingKey(
		...
		public void SeedReceipts_AreIdempotentDistinctAndNodeExact()
		{
			...
			int count;
			bool changed;
			ClassicAssert.IsTrue(KingdomResearchRules.TryApplySeedReceipt("", "salt", "rite:Mechanimists",
				out encoded, out count, out changed));
			ClassicAssert.IsTrue(changed);
			ClassicAssert.AreEqual(1, count);

			string once = encoded;
			ClassicAssert.IsTrue(KingdomResearchRules.TryApplySeedReceipt(encoded, "salt", "RITE:MECHANIMISTS",
				out encoded, out count, out changed));
			ClassicAssert.IsFalse(changed);
			ClassicAssert.AreEqual(once, encoded);
			ClassicAssert.AreEqual(1, count);

			ClassicAssert.IsTrue(KingdomResearchRules.TryApplySeedReceipt(encoded, "salt", "rite:Barathrumites",
				out encoded, out count, out changed));
			ClassicAssert.IsTrue(changed);
			ClassicAssert.AreEqual(2, count);

			ClassicAssert.IsTrue(KingdomResearchRules.TryApplySeedReceipt(encoded, "salt=deep", "rite:Mechanimists",
				out encoded, out count, out changed));
			ClassicAssert.AreEqual(1, count);
			ClassicAssert.AreEqual(2, KingdomResearchRules.SeedReceiptCount(encoded, "salt"),
				"length-prefixed node identities must not count a longer node sharing its prefix");

			string capped = encoded;
			ClassicAssert.IsTrue(KingdomResearchRules.TryApplySeedReceipt(capped, "salt", "rite:Seekers",
				out encoded, out count, out changed));
			ClassicAssert.IsFalse(changed, "a third source needs no row once the 50% floor is represented");
			ClassicAssert.AreEqual(KingdomResearchRules.MaxSeedSourcesPerNode, count);
			ClassicAssert.AreEqual(capped, encoded);
		}
		...
		public void SeedReceipts_RefuseUnboundedGrowth()
		{
			...
			for (int i = 0; i < KingdomResearchRules.MaxSeedReceiptRows; i++)
			{
				int count;
				bool changed;
				ClassicAssert.IsTrue(KingdomResearchRules.TryApplySeedReceipt(encoded, "node" + i,
			...
			int refusedCount;
			bool refusedChanged;
			ClassicAssert.IsFalse(KingdomResearchRules.TryApplySeedReceipt(encoded, "overflow", "rite:last",
				out refused, out refusedCount, out refusedChanged));
			ClassicAssert.AreEqual(encoded, refused);
			ClassicAssert.IsFalse(refusedChanged);
		}
		...
		public void SeedReceipts_IgnoreSpoofsAndSanitizeMalformedRows()
		{
			...
			ClassicAssert.AreEqual(0, KingdomResearchRules.SeedReceiptCount(spoofed, "salt"));
			ClassicAssert.AreEqual(0, KingdomResearchRules.SeededBySources(20, 0,
				KingdomResearchRules.SeedReceiptCount(spoofed, "salt")));

			string updated;
			int count;
			bool changed;
			ClassicAssert.IsTrue(KingdomResearchRules.TryApplySeedReceipt(spoofed, "salt", "rite:Oozes",
				out updated, out count, out changed));
			ClassicAssert.IsTrue(changed);
			ClassicAssert.AreEqual(1, count);
			ClassicAssert.AreEqual(KingdomResearchRules.SeedReceiptKey("salt", "rite:oozes"), updated);
		}
		...
		public void SeedReceipts_RejectAnOversizedDecodedStore()
		{
			...
			string oversized = string.Join(KingdomZoningRules.RosterSeparator.ToString(),
				rows.ToArray());
			ClassicAssert.AreEqual(0, KingdomResearchRules.SeedReceiptCount(oversized, "node0"));

			string updated;
			int count;
			bool changed;
			ClassicAssert.IsFalse(KingdomResearchRules.TryApplySeedReceipt(oversized, "target", "rite:last",
				out updated, out count, out changed));
			ClassicAssert.AreEqual(oversized, updated);
		...
		{
			ClassicAssert.IsNull(KingdomResearchRules.SeedReceiptKey(
				new string('n', KingdomResearchRules.MaxSeedReceiptNodeLength + 1), "rite:source"));
			ClassicAssert.IsNull(KingdomResearchRules.SeedReceiptKey("node", "rite:" +
			...
			ClassicAssert.AreEqual(0, KingdomResearchRules.SeedReceiptCount(oversized, "node"));
			string updated;
			int count;
			bool changed;
			ClassicAssert.IsFalse(KingdomResearchRules.TryApplySeedReceipt(oversized, "node", "rite:source",
				out updated, out count, out changed));
			ClassicAssert.AreEqual(oversized, updated);
		}
		...
		public void ReceiptCap_PreservesTheLastDurableTargetFloor()
		{
			...
			for (int i = 0; i < KingdomResearchRules.MaxSeedReceiptRows - 1; i++)
			{
				int unrelatedCount;
				bool changed;
				ClassicAssert.IsTrue(KingdomResearchRules.TryApplySeedReceipt(encoded, "other" + i,
					"rite:faction" + i, out encoded, out unrelatedCount, out changed));
			}
			int firstCount;
			bool firstChanged;
			ClassicAssert.IsTrue(KingdomResearchRules.TryApplySeedReceipt(encoded, "target", "rite:Oozes",
				out encoded, out firstCount, out firstChanged));
			ClassicAssert.AreEqual(1, firstCount);

			string refused;
			int refusedCount;
			bool refusedChanged;
			ClassicAssert.IsFalse(KingdomResearchRules.TryApplySeedReceipt(encoded, "target", "rite:Fungi",
				out refused, out refusedCount, out refusedChanged));
			int durable = KingdomResearchRules.DurableSeedSourceCount(firstCount, -1);
			ClassicAssert.AreEqual(1, durable);
			ClassicAssert.AreEqual(KingdomResearchRules.EffortTicks(20) / 4,
		...
		public void CrossCityTeaching_UsesTheOtherCityIdentityAsItsStableSource()
		{
			...
			int teachingEnd = source.IndexOf("\n\t\tprivate static string SourceName", teaching,
				StringComparison.Ordinal);
			ClassicAssert.Greater(teachingEnd, teaching);
		...
		public void RuntimeSeedReceiptRefusesBeforeWriteAndPublishesBeforeBenchCredit()
		{
			...
			int seedEnd = source.IndexOf("private static bool SeedBySources(", seed,
				StringComparison.Ordinal);
			ClassicAssert.GreaterOrEqual(seed, 0);
			ClassicAssert.Greater(seedEnd, seed);
			...
			int credit = seedBody.IndexOf("sourceCount > 0 && SeedBySources(",
				StringComparison.Ordinal);
			ClassicAssert.GreaterOrEqual(gate, 0);
			ClassicAssert.Greater(receipt, gate, "all semantic refusals must precede receipt mutation");
			ClassicAssert.Greater(credit, receipt, "durable receipt must precede bench credit");
			...
			int applyEnd = source.IndexOf("private static int SeedSourceCount(", apply,
				StringComparison.Ordinal);
			ClassicAssert.GreaterOrEqual(apply, 0);
			ClassicAssert.Greater(applyEnd, apply);
			...
			ClassicAssert.GreaterOrEqual(merge, 0);
			ClassicAssert.Greater(changed, merge);
			ClassicAssert.Greater(write, changed);
			ClassicAssert.Greater(reproof, write);
		...
		public void FounderLedger_ExistsBeforeFoundingAndIsClearedForKingdomSuccession()
		{
			...
			int resetEnd = source.IndexOf("\n\t\tprivate static void RevealRealmGround", reset,
				StringComparison.Ordinal);
			ClassicAssert.Greater(resetEnd, reset);
