...
namespace ThousandAndFirst.Tests
{
	...
	public sealed class KingdomSubsidenceReleaseCodecTests
	{
		...
		public void Sr2RoundTripRetainsEveryIntentOrReleasedTupleField(bool released, string last, string line)
		{
			...
			if (released) Assert.IsTrue(KingdomSubsidenceRungRules.TryProveRelease(plan, 0, true,
			...
			Assert.AreEqual(released ? KingdomSubsidenceReleasePhase.Released
			...
			Assert.AreEqual(last, decoded.Works[0].ReleaseBefore.LastCompletedId);
			Assert.AreEqual(line, decoded.Works[0].ReleaseBefore.Line);
			Assert.IsNull(decoded.Works[0].ReleaseAfter.Id); Assert.IsNull(decoded.Works[0].ReleaseAfter.Line);
			Assert.AreEqual(untouched, RungFixture.Wire(physical));
			Assert.AreEqual(KingdomSubsidenceReleasePhase.Pending, decoded.Works[1].ReleasePhase);
			Assert.IsFalse(KingdomSubsidenceRungRules.ReleasedComplete(decoded));
		}
		...
		public void WorkWithAndPlanReplaceKeepExactReleaseProofAndPhysicalFields(bool released)
		{
			...
			Assert.AreSame(row.ReleaseBefore, copy.ReleaseBefore); Assert.AreSame(row.ReleaseAfter, copy.ReleaseAfter);
			Assert.AreSame(row.Roofs[0], copy.Roofs[0]); Assert.AreEqual(row.ReleasePhase, copy.ReleasePhase);
			Assert.AreEqual(RungFixture.Wire(plan), RungFixture.Wire(plan.Replace(0, copy)));
			Assert.AreSame(plan.Works[1], plan.Replace(0, copy).Works[1]);
			Assert.IsFalse(TryVersion(typeof(KingdomSubsidenceRungCodec), plan, 1, out string refused));
			Assert.IsNull(refused, "the old writer cannot erase an armed or released receipt");
		}
		...
		public void RealCanonicalSr1ReadsPendingWithoutInventingReleaseEvenForMultipleProvedWorks(bool complete)
		{
			...
			Assert.IsTrue(TryVersion(typeof(KingdomSubsidenceRungCodec), original, 1, out string legacy));
			StringAssert.StartsWith("sr1:", legacy); Assert.AreEqual(0x31525354, BitConverter.ToInt32(Bytes(legacy), 0));
			Assert.IsTrue(KingdomSubsidenceRungCodec.TryDecode(legacy, out KingdomSubsidenceRungPlan decoded));
			Assert.AreEqual(2, decoded.Works.Count); Assert.AreEqual(complete, KingdomSubsidenceRungRules.PhysicalComplete(decoded));
			foreach (KingdomSubsidenceRungWork row in decoded.Works)
			{
				Assert.AreEqual(KingdomSubsidenceReleasePhase.Pending, row.ReleasePhase);
				Assert.IsNull(row.ReleaseBefore); Assert.IsNull(row.ReleaseAfter);
			}
			Assert.IsFalse(KingdomSubsidenceRungRules.ReleasedComplete(decoded));
			Assert.IsTrue(TryVersion(typeof(KingdomSubsidenceRungCodec), decoded, 1, out string canonical));
			Assert.AreEqual(legacy, canonical);
			Assert.AreEqual(RungFixture.Wire(original), RungFixture.Wire(decoded));
			Assert.AreEqual(Bytes(legacy).Length + decoded.Works.Count, Bytes(RungFixture.Wire(decoded)).Length);
		}
		...
		public void EveryOuterVersionAndCopyPreservesRawSr1AndAnyAlreadyPublishedReport(int version, bool published)
		{
			...
			Assert.IsTrue(TryVersion(typeof(KingdomSubsidenceStepCodec), book, version, out string wire));
			...
			Assert.IsTrue(TryVersion(typeof(KingdomSubsidenceStepCodec), decoded, version, out string canonical));
			Assert.AreEqual(wire, canonical);
			...
			Assert.IsTrue(KingdomSubsidenceStepCodec.TryEncode(decoded, out string current));
			StringAssert.StartsWith("ss5:", current);
			Assert.IsTrue(KingdomSubsidenceStepCodec.TryDecode(current, out decoded));
			Assert.AreEqual(rung, decoded.Active.RungModel); Assert.AreEqual(report, decoded.Active.RungReportModel);
			Assert.IsTrue(KingdomSubsidenceStepRules.TryReadRungPlan(decoded, out KingdomSubsidenceRungPlan plan));
			...
			Assert.IsFalse(KingdomSubsidenceStepRules.TryCheckpoint(decoded, decoded.Active.AnchorTick, out _));
			Assert.IsFalse(KingdomSubsidenceStepRules.TryRetire(decoded, decoded.Active.DueTick, out _));
			if (published)
			{
				Assert.IsTrue(KingdomSubsidenceReportCodec.TryDecode(report, out KingdomSubsidenceReportPlan telling));
				Assert.IsTrue(KingdomSubsidenceReportRules.Complete(telling));
			}
		}
		...
		{
			Assert.IsFalse(TryVersion(typeof(KingdomSubsidenceStepCodec), LegacyBook(true), version, out string refused));
		...
		public void EveryOuterLayoutRetainsSr2ProofAcrossOperationAndBookCopies(int version, bool released)
		{
			...
			Assert.IsTrue(KingdomSubsidenceStepCodec.TryEncode(book, out wire));
			Assert.IsTrue(KingdomSubsidenceStepCodec.TryDecode(wire, out book));
			Assert.AreEqual(rung, book.Active.RungModel);
			Assert.IsTrue(KingdomSubsidenceStepRules.TryReadRungPlan(book, out KingdomSubsidenceRungPlan decoded));
			EqualReceipt(plan.Works[0].ReleaseBefore, decoded.Works[0].ReleaseBefore);
			EqualReceipt(plan.Works[0].ReleaseAfter, decoded.Works[0].ReleaseAfter);
		}
		...
		{
			Assert.IsTrue(TryVersion(typeof(KingdomSubsidenceRungCodec), Fixture(true), 1, out string wire));
		...
		public void Sr1TrailingWhitespaceOverlongStringAndWrongVersionCannotBeNormalizedIntoSr2()
		{
			...
			Assert.Less(bytes[4], 128); Array.Copy(bytes, 0, longer, 0, 4);
		...
		public void Sr1EquivalentNonzeroPaddingBitsStillFailOriginalVersionCanonicalCheck()
		{
			...
			{
				Assert.IsTrue(TryVersion(typeof(KingdomSubsidenceRungCodec), RungFixture.Plan(
		...
		public void EveryReceiptTruncationAndTrailingByteRefusesWithoutPublishingPartialPlan(bool released)
		{
			...
			Assert.AreEqual(wire, RungFixture.Wire(RungFixture.RoundTrip(plan)));
		}
		...
		private static KingdomSubsidenceRungPlan Arm(KingdomSubsidenceRungPlan plan, string last, string line)
		{
			...
			Assert.IsTrue(KingdomSubsidenceRungRules.TryArmRelease(plan, 0, true, observed, out KingdomSubsidenceRungPlan next));
			Assert.AreSame(observed, next.Works[0].ReleaseBefore);
			return next;
		}
		private static KingdomSubsidenceStepBook LegacyBook(bool published)
		{
			...
			Assert.IsTrue(TryVersion(typeof(KingdomSubsidenceRungCodec), p, 1, out string legacy));
		...
		private static bool TryVersion(Type codec, object model, int version, out string wire)
		{
			...
			Assert.IsNotNull(writer); object[] arguments = { model, version, null };
		...
		private static void EqualReceipt(KingdomSubsidenceWearReceipt a, KingdomSubsidenceWearReceipt b)
		{
			...
			Assert.AreEqual(a.Phase, b.Phase); Assert.AreEqual(a.Id, b.Id); Assert.AreEqual(a.Cause, b.Cause);
			Assert.AreEqual(a.BeforeWear, b.BeforeWear); Assert.AreEqual(a.AfterWear, b.AfterWear);
			Assert.AreEqual(a.Wear, b.Wear); Assert.AreEqual(a.LastCause, b.LastCause);
			Assert.AreEqual(a.LastCompletedId, b.LastCompletedId); Assert.AreEqual(a.Line, b.Line);
			Assert.AreEqual(a.MessageState, b.MessageState);
		}
		private static Dictionary<string, int> ReceiptFlags(string wire)
		{
			...
			using (var reader = new BinaryReader(stream))
			{
				...
				Assert.AreEqual((byte)KingdomSubsidenceReleasePhase.Intent, reader.ReadByte());
		...
		{
			Assert.IsFalse(KingdomSubsidenceRungCodec.TryDecode(wire, out KingdomSubsidenceRungPlan value));
			Assert.IsNull(value);
		}
	}
}
