...
namespace ThousandAndFirst.DevTests
{
	...
	public sealed class KingdomPolityEndpointObservationRulesTests
	{
		...
		{
			ClassicAssert.IsTrue(KingdomPolityEndpointObservationRules.TryGuard(
		...
		{
			ClassicAssert.IsTrue(KingdomPolityEndpointObservationRules.TryCondition(
				KingdomPolityTestData.Realm, Settlement,
			...
			ClassicAssert.IsTrue(KingdomPolityEndpointObservationRules.TryCondition(
				...
				out failure), failure);
			ClassicAssert.AreEqual(first.CauseRef, reordered.CauseRef);
			ClassicAssert.AreEqual(first.LocusRef, reordered.LocusRef);
		}
		...
		public void UnwitnessedAndMalformedRowsFailClosedWithoutInventingCause()
		{
			ClassicAssert.IsTrue(KingdomPolityEndpointObservationRules.TryCondition(
				...
				out string failure), failure);
			ClassicAssert.IsNull(absent);
			ClassicAssert.IsFalse(KingdomPolityEndpointObservationRules.TryGuard(
				KingdomPolityTestData.Realm, Settlement, new[] { "zone-a" },
				new long[] { 1L, 2L }, new[] { 1 }, out _, out failure));
			ClassicAssert.IsFalse(KingdomPolityEndpointObservationRules.TryCondition(
		...
		public void GuardAndPatrolFactsFreezeIntoDistinctAmbientTransactions()
		{
				...
				{ KingdomPolityCohortPurpose.Guard, KingdomPolityCohortPurpose.Patrol })
			{
				...
				ClassicAssert.AreEqual(purpose, transaction.Purpose);
				ClassicAssert.AreEqual(work.CauseRef, transaction.FactRefs[0]);
			}
		}
	}
}
