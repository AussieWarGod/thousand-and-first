...
namespace ThousandAndFirst.Tests
{
	public class KingdomConstructionInputRegistryRulesTests
	{
		...
		public void RoutedReceiptBindsOuterJobIntentClaimsAndWire()
		{
			...
			ClassicAssert.IsTrue(KingdomConstructionRules.UpdateInputReceipt(ref job, receipt));
			...
			ClassicAssert.AreEqual(57, wire.Split('\n')[1].Split('|').Length);
			List<KingdomConstructionJob> decoded;
			ClassicAssert.IsTrue(KingdomConstructionRules.TryDecode(wire, out decoded));
			KingdomConstructionInputReceipt reloaded;
			ClassicAssert.IsTrue(KingdomConstructionRules.TryGetInputReceipt(decoded[0], out reloaded));
			ClassicAssert.AreEqual(receipt.PlanDigest, reloaded.PlanDigest);

			KingdomConstructionJob tampered = job.Copy();
			tampered.Payload = "repriced";
			ClassicAssert.IsFalse(KingdomConstructionRules.ValidJob(tampered));
			tampered = job.Copy();
		...
		public void ActiveReceiptRequiresOnePureCasAndCommitIsAtomicFunding()
		{
			...
			KingdomConstructionJob attached = KingdomConstructionRules.Transition(current,
				current.Phase, 11L);
			ClassicAssert.IsTrue(KingdomConstructionRules.UpdateInputReceipt(ref attached, receipt));
			ClassicAssert.IsTrue(KingdomConstructionRules.ValidRegistryUpdate(current, attached));
			...
			KingdomConstructionJob next = KingdomConstructionRules.Transition(attached,
				attached.Phase, 12L);
			ClassicAssert.IsTrue(KingdomConstructionRules.UpdateInputReceipt(ref next, reserved));
			ClassicAssert.IsTrue(KingdomConstructionRules.ValidRegistryUpdate(attached, next));
			KingdomConstructionJob unrelated = KingdomConstructionRules.Transition(next,
				next.Phase, 13L, "edited without receipt CAS");
			ClassicAssert.IsFalse(KingdomConstructionRules.ValidRegistryUpdate(next, unrelated));
			...
			ClassicAssert.IsTrue(KingdomConstructionRules.UpdateInputReceipt(ref beforeCommit, closing));
			...
			ClassicAssert.IsTrue(KingdomConstructionRules.UpdateInputReceipt(ref funded, committed));
			ClassicAssert.IsTrue(KingdomConstructionRules.ValidJob(funded));
			ClassicAssert.IsTrue(KingdomConstructionRules.ValidRegistryUpdate(beforeCommit, funded));
			...
			ClassicAssert.IsTrue(KingdomConstructionRules.ValidJob(projected),
				"terminal input proof must not freeze ordinary post-funding projection state");
			ClassicAssert.IsTrue(KingdomConstructionRules.ValidRegistryUpdate(funded, projected));
			...
			ClassicAssert.IsFalse(KingdomConstructionRules.ValidJob(unpaid));
		}
		...
		public void TerminalCompactionRetainsFinalRoutedReplayHash()
		{
			...
			ClassicAssert.IsNull(normalized[0].InputReceipt);
			ClassicAssert.AreEqual(finalHash, normalized[0].InputReceiptHash);
			ClassicAssert.IsTrue(KingdomConstructionRules.ValidJob(normalized[0]));
		}
		...
		private static KingdomConstructionInputReceipt NewReceipt(KingdomConstructionJob job)
		{
			...
			string digest;
			ClassicAssert.IsTrue(KingdomConstructionRules.TryInputIntent(job, 0,
			...
			ClassicAssert.IsTrue(KingdomConstructionInputRules.TryCreate("receipt-1", job.Id,
			...
			KingdomConstructionInputTxPhase next)
		{
			...
			ClassicAssert.IsTrue(KingdomConstructionInputRules.TryTransitionTransaction(value,
		...
		{
			KingdomConstructionInputReceipt updated; KingdomConstructionInputFault fault;
			ClassicAssert.IsTrue(KingdomConstructionInputRules.TryTransitionSource(value,
		...
		{
			KingdomConstructionInputReceipt updated; KingdomConstructionInputFault fault;
			ClassicAssert.IsTrue(KingdomConstructionInputRules.TryTransitionCargo(value,
		...
		{
			KingdomConstructionInputReceipt updated; KingdomConstructionInputFault fault;
			ClassicAssert.IsTrue(KingdomConstructionInputRules.TryTransitionCargoWithEvidence(value,
