...
namespace ThousandAndFirst.Tests
{
	public class KingdomRaidIncidentRulesTests
	{
		...
		public void ExactAuthoredSourceMintsOneStableGrievanceAndOneStableIncident()
		{
			...
			ClassicAssert.AreEqual(KingdomRaidIncidentRules.GrievanceId("source-a"), warning.ObjectId);
			ClassicAssert.AreEqual(KingdomRaidIncidentRules.IncidentId(warning.ObjectId),
				warning.ObjectMarker);
			ClassicAssert.IsTrue(KingdomRaidIncidentRules.TryApply(ledger, warning, out ledger));
			ClassicAssert.AreEqual(1, ledger.Grievances.Count);
			ClassicAssert.AreEqual(1, ledger.Incidents.Count);
			ClassicAssert.AreEqual(warning.ObjectMarker, ledger.ActiveIncidentId);
			ClassicAssert.AreEqual("specific authored evidence", ledger.Incidents[0].CauseSnapshot);
			ClassicAssert.AreEqual("Snapjaws", ledger.Incidents[0].AttackerFactionId);
			ClassicAssert.AreEqual("zone-a", ledger.Incidents[0].TargetZoneId);
			ClassicAssert.AreEqual(KingdomRaidIncidentState.Rumored, ledger.Incidents[0].State);
			ClassicAssert.AreEqual(0L, ledger.Incidents[0].DeliveredTick);
			ClassicAssert.AreEqual(0L, ledger.Incidents[0].DueTick);
			ClassicAssert.AreEqual(100L, ledger.Incidents[0].DemandLeadTicks);
			ClassicAssert.AreEqual(KingdomRaidChannelState.AwaitingDelivery,
				ledger.Incidents[0].ChannelState);
			ClassicAssert.AreEqual("stores", ledger.Incidents[0].ObjectiveCode);
			ClassicAssert.AreEqual(24, ledger.Incidents[0].MaximumPlunder);
			ClassicAssert.AreEqual("salt-road scouts", ledger.Incidents[0].ReachRule);
			ClassicAssert.IsFalse(KingdomRaidIncidentRules.TryApply(ledger, warning, out _));
			ClassicAssert.IsTrue(KingdomRaidIncidentRules.SourceConsumed(ledger, "source-a"));
		}
		...
		public void RumorPhysicalDeliveryAcknowledgementLossAndDeadlineStayDistinct()
		{
			...
			ClassicAssert.AreEqual(KingdomRaidIncidentState.Rumored, incident.State);
			ClassicAssert.AreEqual(0L, incident.DueTick);
			...
			ClassicAssert.AreEqual(KingdomRaidIncidentState.Rumored, incident.State);
			ClassicAssert.AreEqual(KingdomRaidChannelState.Issued, incident.ChannelState);
			ClassicAssert.AreEqual(0L, incident.DueTick);

			ledger = Apply(ledger, Acknowledgement(incident, 12L, 112L));
			incident = KingdomRaidIncidentRules.Active(ledger);
			ClassicAssert.AreEqual(KingdomRaidIncidentState.Warned, incident.State);
			ClassicAssert.AreEqual(112L, incident.DueTick);
			...
			incident = KingdomRaidIncidentRules.Active(ledger);
			ClassicAssert.AreEqual(KingdomRaidChannelState.RedeliveryQueued, incident.ChannelState);
			ClassicAssert.AreEqual(0L, incident.DueTick);
			ClassicAssert.AreEqual(92L, incident.RemainingLeadTicks);

			ledger = Apply(ledger, Delivery(incident, 30L));
			incident = KingdomRaidIncidentRules.Active(ledger);
			ClassicAssert.AreEqual(2, incident.ChannelRevision);
			...
			ClassicAssert.IsTrue(KingdomRaidIncidentRules.CanPublish(ledger,
			...
			ClassicAssert.IsTrue(KingdomRaidIncidentRules.CanPublish(ledger, order));
		}
		...
		public void MultipleAuthoredSourcesQueueWithoutLossAndPromoteDeterministically()
		{
			...
			ClassicAssert.AreEqual(2, ledger.Grievances.Count);
			ClassicAssert.AreEqual(2, ledger.Incidents.Count);
			KingdomRaidIncident first = KingdomRaidIncidentRules.Active(ledger);
			KingdomRaidIncident second = ledger.Incidents[1];
			ClassicAssert.AreEqual(KingdomRaidIncidentState.Queued, second.State);
			...
			ClassicAssert.AreEqual(KingdomRaidIncidentState.Cancelled, ledger.Incidents[0].State);
			ClassicAssert.AreEqual(second.Id, ledger.ActiveIncidentId);
			ClassicAssert.AreEqual(KingdomRaidIncidentState.Rumored, ledger.Incidents[1].State);
			ClassicAssert.AreEqual(0L, ledger.Incidents[1].DueTick,
				"promotion carries rumor authority only; delivery has not been acknowledged");
			ClassicAssert.AreEqual(100L, ledger.Incidents[1].DemandLeadTicks);
			ClassicAssert.AreEqual(KingdomRaidChannelState.AwaitingDelivery,
				ledger.Incidents[1].ChannelState);
			ClassicAssert.IsTrue(KingdomRaidIncidentRules.ValidLedger(ledger));
		}
		...
		public void TributeTalkFightAndFortifyHaveExplicitAtomicDomainResults()
		{
			...
			ClassicAssert.IsFalse(KingdomRaidIncidentRules.TryApply(tribute, unpaid, out _));
			unpaid.WaterProved = 6;
			tribute = Apply(tribute, unpaid);
			ClassicAssert.AreEqual(KingdomRaidResolution.TributePaid,
				tribute.Incidents[0].Resolution);
			ClassicAssert.IsNull(tribute.ActiveIncidentId);
			...
			ClassicAssert.IsTrue(talk.Incidents[0].TalkObligation);
			ClassicAssert.IsTrue(KingdomRaidIncidentRules.HasTalkObligation(talk, "Snapjaws"));
			talk = Apply(talk, Warning("city-a", "later-provocation", 50L, 12));
			ClassicAssert.AreEqual(talk.Incidents[1].Id,
				talk.Incidents[0].TalkObligationDischargedBy);
			ClassicAssert.IsFalse(KingdomRaidIncidentRules.HasTalkObligation(talk, "Snapjaws"));
			...
			ClassicAssert.AreEqual(KingdomRaidIncidentState.FightCommitted,
				KingdomRaidIncidentRules.Active(fight).State);
			ClassicAssert.AreEqual(KingdomRaidResponse.Fight,
			...
			ClassicAssert.AreEqual(KingdomRaidIncidentState.Fortified,
				KingdomRaidIncidentRules.Active(fortify).State);
			ClassicAssert.AreEqual("R1;101=2[7,9];202=1[]",
				KingdomRaidIncidentRules.Active(fortify).DefenceCommitment);
			ClassicAssert.AreEqual(3, KingdomRaidIncidentRules.Active(fortify).DefenceEstimate);
		...
		public void FortifyReservationIsCanonicalAndCrewCannotServeTwoWorks()
		{
			...
			ClassicAssert.IsTrue(KingdomRaidIncidentRules.TryEncodeDefenceReservations(rows,
				out string commitment, out int total));
			ClassicAssert.AreEqual("R1;101=2[7,9];202=1[]", commitment);
			ClassicAssert.AreEqual(3, total);
			ClassicAssert.IsTrue(KingdomRaidIncidentRules.TryDecodeDefenceReservations(commitment,
			...
			ClassicAssert.IsFalse(KingdomRaidIncidentRules.TryEncodeDefenceReservations(rows,
				out _, out _), "one resident row cannot reserve two defensive jobs");
			ClassicAssert.IsFalse(KingdomRaidIncidentRules.TryDecodeDefenceReservations(
		...
		public void AttackCannotRecordPlunderUntilSeparateObjectiveResolutionProof()
		{
			...
			ClassicAssert.IsFalse(KingdomRaidIncidentRules.TryApply(ledger, attack, out _),
				"an attack operation cannot pre-credit plunder before objective contact");
			attack.PlunderProved = 0;
			ledger = Apply(ledger, attack);
			incident = KingdomRaidIncidentRules.Active(ledger);
			ClassicAssert.AreEqual(KingdomRaidIncidentState.Active, incident.State);
			ClassicAssert.AreEqual(0, incident.PlunderProved);
			ClassicAssert.AreEqual("exact-store-id", incident.ObjectiveObjectId);
			ClassicAssert.AreEqual(4, incident.ObjectiveX);
			ClassicAssert.AreEqual(5, incident.ObjectiveY);
			...
			ClassicAssert.AreEqual(3, ledger.Incidents[0].PlunderProved);
			ClassicAssert.AreEqual(KingdomRaidResolution.StoresPlundered,
				ledger.Incidents[0].Resolution);
			KingdomRaidIncident recovery = ledger.Incidents[0];
			ClassicAssert.AreEqual(KingdomRaidRecoveryState.Offered, recovery.RecoveryState);
			ClassicAssert.AreEqual(KingdomRaidIncidentRules.RecoveryQuestId(recovery.Id),
			...
			ClassicAssert.AreEqual(KingdomRaidRecoveryState.Ready,
			...
			ClassicAssert.AreEqual(KingdomRaidRecoveryState.Resolved,
				ledger.Incidents[0].RecoveryState);
		}
		...
		public void RaidLedgerV1AndV2FixturesMigrateAndV3OrFutureRoundTripsExactly()
		{
			...
			ClassicAssert.AreEqual(734, v1.Length, "PIN_RAID_V1_LENGTH");
			ClassicAssert.AreEqual("702aaa4e997ef4b96fbc1c4d1989de7a3d62dd99bb46eb4e89d81c82aea35d03",
				Sha256(v1), "PIN_RAID_V1_SHA256");
			KingdomRaidLedger migrated = KingdomLifecycleWireCodec.ReadRaidLedgerFixture(v1);
			ClassicAssert.AreEqual(KingdomRaidLedger.CurrentVersion, migrated.Version);
			ClassicAssert.AreEqual(KingdomRaidIncidentState.Warned, migrated.Incidents[0].State);
			ClassicAssert.AreEqual(KingdomRaidChannelState.RedeliveryQueued,
				migrated.Incidents[0].ChannelState);
			ClassicAssert.AreEqual(0L, migrated.Incidents[0].DueTick);
			...
			ClassicAssert.AreEqual("da982eef9f8c70fcfb677cb33e5c084b549e7c670de55974bf5d050fd2c47a60",
				Sha256(v2), "PIN_RAID_V2_SHA256");
			KingdomRaidLedger reopened = KingdomLifecycleWireCodec.ReadRaidLedgerFixture(v2);
			ClassicAssert.AreEqual(KingdomRaidLedger.CurrentVersion, reopened.Version);
			ClassicAssert.AreEqual(KingdomRaidIncidentState.ConfrontationReady,
				reopened.Incidents[0].State);
			ClassicAssert.AreEqual(KingdomRaidResponse.None, reopened.Incidents[0].Response);
			ClassicAssert.AreEqual(0, reopened.Incidents[0].DefenceReservationVersion);
			ClassicAssert.AreEqual(0, reopened.Incidents[0].DefenceReservations.Count);
			...
			ClassicAssert.AreEqual(future.Version, retained.Version);
			...
			ClassicAssert.IsNull(KingdomRaidIncidentRules.Active(retained));
		}
		...
		public void LedgerRejectsSemanticEvidenceAndObjectiveTampering()
		{
			...
			ClassicAssert.IsFalse(KingdomRaidIncidentRules.ValidLedger(changedCause));

			KingdomRaidLedger changedClock = KingdomRaidIncidentRules.Copy(original);
			changedClock.Incidents[0].DueTick = 1L;
			ClassicAssert.IsFalse(KingdomRaidIncidentRules.ValidLedger(changedClock));
			...
			ClassicAssert.IsFalse(KingdomRaidIncidentRules.ValidLedger(inventedObjective));
			...
			ClassicAssert.IsFalse(KingdomRaidIncidentRules.TryApply(original, prematureAttack, out _),
		...
		public void LegacyStandingDerivedEvidenceIsArchivedRawWithoutMintingCause()
		{
			...
			ClassicAssert.IsTrue(ledger.LegacyEvidenceArchived);
			ClassicAssert.AreEqual(1, ledger.LegacyRaidState);
			ClassicAssert.AreEqual("Snapjaws", ledger.LegacyFaction);
			ClassicAssert.AreEqual(400L, ledger.LegacyDueTick);
			ClassicAssert.AreEqual(350L, ledger.LegacyLastTick);
			ClassicAssert.AreEqual(3, ledger.LegacyTimesDeferred);
			ClassicAssert.AreEqual(0, ledger.Grievances.Count);
			ClassicAssert.AreEqual(0, ledger.Incidents.Count);
			ClassicAssert.IsNull(ledger.ActiveIncidentId);
			ClassicAssert.IsFalse(KingdomRaidIncidentRules.TryApply(ledger, legacy, out _));
		}
		...
		public void ArchiveV2ColdLoadKeepsRawLegacyFieldsAndCreatesNoGrievance()
		{
			...
			ClassicAssert.IsTrue(KingdomArchivedSettlementCodec.TryEncodePreviousV2ForTests(old,
				out byte[] v2, out string failure), failure);
			ClassicAssert.AreEqual(39098, v2.Length);
			ClassicAssert.AreEqual(
				"a0a5c95781e8893e4a7c1d6ade09ef05dbb6eaa395919a8b012f3e57fa9c987c",
				Sha256(v2));
			ClassicAssert.AreEqual(KingdomArchivedSettlementCodec.PreviousVersion,
			...
			ClassicAssert.IsTrue(KingdomRaidIncidentRules.ValidLedger(loaded.LifecycleBook.RaidLedger));
			ClassicAssert.AreEqual(0, loaded.LifecycleBook.RaidLedger.Grievances.Count);
			ClassicAssert.AreEqual(0, loaded.LifecycleBook.RaidLedger.Incidents.Count);
			ClassicAssert.AreEqual(1, loaded.RaidState);
			ClassicAssert.AreEqual("Snapjaws", loaded.RaidFactionName);
			ClassicAssert.AreEqual(400L, loaded.RaidDueTick);
			ClassicAssert.AreEqual(350L, loaded.LastRaidTick);
			ClassicAssert.AreEqual(3, loaded.RaidTimesDeferred);
		}
		...
		public void ArchiveV3RoundTripRetainsOneIncidentAndRejectsDuplicateEvidence()
		{
			...
			ClassicAssert.IsTrue(KingdomArchivedSettlementCodec.TryDecode(payload,
				out KingdomSettlement loaded, out int future, out failure), failure);
			ClassicAssert.AreEqual(0, future);
			ClassicAssert.AreEqual(1, loaded.LifecycleBook.RaidLedger.Grievances.Count);
			ClassicAssert.AreEqual(1, loaded.LifecycleBook.RaidLedger.Incidents.Count);
			ClassicAssert.AreEqual(warning.ObjectMarker,
				loaded.LifecycleBook.RaidLedger.ActiveIncidentId);
			ClassicAssert.IsFalse(KingdomRaidIncidentRules.TryApply(
			...
			ClassicAssert.IsFalse(KingdomRaidIncidentRules.ValidLedger(loaded.LifecycleBook.RaidLedger));
			ClassicAssert.IsTrue(KingdomArchivedSettlementCodec.TryEncodeUncheckedCurrentForTests(
				loaded, out byte[] malformed, out failure), failure);
			ClassicAssert.IsFalse(KingdomArchivedSettlementCodec.TryDecode(malformed,
		...
		private static KingdomSettlement Settlement(string id)
		{
			...
			ClassicAssert.IsTrue(KingdomLifecycleRules.BindSettlementIdentity(value.LifecycleBook,
			...
			KingdomLifecycleOperation operation)
		{
			...
			ClassicAssert.IsTrue(KingdomRaidIncidentRules.ValidLedger(after));
			return after;
		}
