...
namespace ThousandAndFirst.Tests
{
	internal static class ChronicleCapacityFixture
	{
		...
		internal static string Registry(int count, bool alternate = false)
		{
			...
			ClassicAssert.IsTrue(KingdomChronicleReceiptRules.TryWriteRegistry(rows, out string wire, out var fault), fault.ToString());
			return wire;
		}
		internal static string Fingerprint(string id = "new:event", string text = "A departure", long tick = 10)
		{
			ClassicAssert.IsTrue(KingdomChronicleCapacityRules.TryFingerprint(Realm, Settlement, id, text, tick, out string value));
			return value;
		}
	...
	public sealed class KingdomChronicleCapacityRulesTests
	{
		...
		public void FullCanonicalRegistryProducesExactBoundedWitnessWithoutChangingAnyRow()
		{
			...
			ClassicAssert.IsTrue(KingdomChronicleCapacityRules.TryObserve(shape, "new:event", fingerprint, out var witness));
			ClassicAssert.AreEqual(4096, witness.RegistryCount);
			ClassicAssert.IsTrue(KingdomChronicleReceiptRules.IsSha256(witness.RegistryHash));
			ClassicAssert.AreEqual("new:event", witness.EventId); ClassicAssert.AreEqual(fingerprint, witness.Fingerprint);
			ClassicAssert.AreSame(before, shape.String);
			ClassicAssert.IsTrue(KingdomChronicleReceiptRules.TryParseRegistry(shape.String, out var rows, out bool migrated, out _));
			ClassicAssert.IsFalse(migrated); ClassicAssert.AreEqual(4096, rows.Count);
			ClassicAssert.IsTrue(KingdomChronicleReceiptRules.TryWriteRegistry(rows, out string after, out _));
			ClassicAssert.AreEqual(before, after);
			ClassicAssert.IsTrue(KingdomChronicleCapacityRules.TryObserve(shape, "new:event", fingerprint, out var repeat));
			ClassicAssert.AreEqual(witness.RegistryHash, repeat.RegistryHash);
		}
		...
		public void OnlyStringPresenceAcrossAllFiveTablesCanAuthorizeCapacity(int mask)
		{
			...
			ClassicAssert.AreEqual(mask == 1, KingdomChronicleCapacityRules.TryObserve(shape, "new:event",
				ChronicleCapacityFixture.Fingerprint(), out var witness));
			ClassicAssert.AreEqual(mask == 1, witness != null);
			ClassicAssert.AreSame(ChronicleCapacityFixture.Full, shape.String);
			ClassicAssert.AreEqual((mask & 2) != 0, shape.HasInt); ClassicAssert.AreEqual((mask & 8) != 0, shape.HasObject);
		}
		...
		public void MissingMalformedAndLegacyTextNeverBecomesCapacity(string raw)
		{
			...
			ClassicAssert.IsFalse(KingdomChronicleCapacityRules.TryObserve(shape, "new:event", ChronicleCapacityFixture.Fingerprint(), out var witness));
			ClassicAssert.IsNull(witness); ClassicAssert.AreEqual(raw, shape.String);
		}

		[TestCase(0)] [TestCase(1)] [TestCase(4095)]
		public void ValidAvailableCapacityIsNotTerminalRefusal(int count)
		{
			ClassicAssert.IsFalse(KingdomChronicleCapacityRules.TryObserve(ChronicleCapacityFixture.Shape(
				ChronicleCapacityFixture.Registry(count)), "new:event", ChronicleCapacityFixture.Fingerprint(), out var witness));
			ClassicAssert.IsNull(witness);
		}

		[Test]
		public void ExistingEventEvenWithForeignFingerprintIsNeverCapacityRefused()
		{
			ClassicAssert.IsFalse(KingdomChronicleCapacityRules.TryObserve(ChronicleCapacityFixture.Shape(), "capacity-fixture:0",
		...
		public void NoncanonicalAndOversizedRegistryRemainRefusalsWithoutWitness()
		{
			foreach (string raw in new[] { ChronicleCapacityFixture.Full + "\n", ChronicleCapacityFixture.Full.Replace("\n", "\r\n"),
				...
				ClassicAssert.IsFalse(KingdomChronicleCapacityRules.TryObserve(ChronicleCapacityFixture.Shape(raw),
		...
		{
			ClassicAssert.IsTrue(KingdomChronicleCapacityRules.TryObserve(ChronicleCapacityFixture.Shape(), "new:event",
			...
			ClassicAssert.AreNotEqual(first.RegistryHash, second.RegistryHash);
		}

		[Test]
		public void NullObservationAndInvalidEventIdentityNeverAuthorizeCapacity()
		{
			ClassicAssert.IsFalse(KingdomChronicleCapacityRules.TryObserve(null, "new:event", ChronicleCapacityFixture.Fingerprint(), out _));
			foreach (string id in new[] { null, "", "bad\nidentity", new string((char)0xD800, 1), new string('x', 257) })
				ClassicAssert.IsFalse(KingdomChronicleCapacityRules.TryObserve(ChronicleCapacityFixture.Shape(), id,
					ChronicleCapacityFixture.Fingerprint(), out _));
			foreach (string hash in new[] { null, "", new string('A', 64), new string('a', 63) })
				ClassicAssert.IsFalse(KingdomChronicleCapacityRules.TryObserve(ChronicleCapacityFixture.Shape(), "new:event", hash, out _));
		}
		...
		public void DatedFingerprintRejectsInvalidTextIdentityAndClock()
		{
			foreach (string text in new[] { null, "bad\ntext", new string((char)0xD800, 1), new string('x', 4097) })
				ClassicAssert.IsFalse(KingdomChronicleCapacityRules.TryFingerprint(ChronicleCapacityFixture.Realm,
					ChronicleCapacityFixture.Settlement, "new:event", text, 10, out _));
			ClassicAssert.IsFalse(KingdomChronicleCapacityRules.TryFingerprint("foreign", ChronicleCapacityFixture.Settlement, "event", "text", 10, out _));
			ClassicAssert.IsFalse(KingdomChronicleCapacityRules.TryFingerprint(ChronicleCapacityFixture.Realm, "foreign", "event", "text", 10, out _));
			ClassicAssert.IsFalse(KingdomChronicleCapacityRules.TryFingerprint(ChronicleCapacityFixture.Realm, ChronicleCapacityFixture.Settlement, "", "text", 10, out _));
			ClassicAssert.IsFalse(KingdomChronicleCapacityRules.TryFingerprint(ChronicleCapacityFixture.Realm, ChronicleCapacityFixture.Settlement, "event", "text", -1, out _));
			ClassicAssert.IsTrue(KingdomChronicleCapacityRules.TryFingerprint(ChronicleCapacityFixture.Realm, ChronicleCapacityFixture.Settlement,
