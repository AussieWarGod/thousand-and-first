...
namespace ThousandAndFirst.Tests
{
	public sealed class KingdomSubsidenceExecutionClockTests
	{
			...
			long now, KingdomElapsedOptionAction expected)
		{
			...
			ClassicAssert.IsTrue(decision.Valid); ClassicAssert.AreEqual(expected, decision.Action);
			Check(Admitted(), now, 0, true); Check(Admitted(), now, -1, false);
			Check(Admitted(), now, now + 1, false);
			ClassicAssert.AreEqual(KingdomElapsedOptionRules.Encode(prior), KingdomElapsedOptionRules.Encode(decision.Record));
		}
		...
		public void ActiveClockNeedsAnchorUntilExactDepartureRungAndReportAuthority(int state)
		{
			...
			if (state > 0) ClassicAssert.IsTrue(KingdomSubsidenceStepRules.TryAssociate(book, departure, out book));
			if (state > 1) ClassicAssert.IsTrue(KingdomSubsidenceStepRules.TryCredit(book, departure.OperationId,
				state == 5 ? GrowthStage.Town : GrowthStage.City, out book));
			if (state > 2) ClassicAssert.IsTrue(KingdomSubsidenceStepRules.TryReleaseRetired(book, departure.OperationId, out book));
			Check(book, Late, Anchor, true); Check(book, Late, Anchor + 1, false);
			Check(book, Late, Due, state == 3); Check(book, Late, Late, false);
			ClassicAssert.AreEqual(state == 3, KingdomSubsidenceStepRules.TryCheckpoint(book, Due, out _));
		}
			...
			bool committed, bool frozenOption)
		{
			...
			ClassicAssert.IsTrue(KingdomSubsidenceStepRules.TryCheckpoint(book, Anchor, out long proved));
			ClassicAssert.AreEqual(target, proved);
		...
		public void PendingOptionWithoutActiveStepUsesItsOwnCheckpointOracle(int mode)
		{
			KingdomSubsidenceStepBook book = mode == 0 ? Freeze(Admitted(), Anchor)
				: Retired(mode == 1, true);
			ClassicAssert.IsTrue(KingdomSubsidenceOptionIntentRules.TryDecode(book.OptionModel,
			...
			foreach (long raw in new[] { 0L, Anchor, Anchor + 1, Due, Late, Late + 1 })
			{
				...
				if (allowed) ClassicAssert.AreEqual(Late, target);
		...
		public void FrozenFutureDatesRefuseEvenWhenTheRawClockIsInRange(string kind)
		{
			...
			if (kind == "cancellation") ClassicAssert.IsTrue(KingdomSubsidenceStepRules.TryCancel(book, Late, 2, out book));
		...
		public void MaximumWorldTickAndExactTerminalStepNeedNoOverflowingArithmetic()
		{
			...
			ClassicAssert.IsTrue(KingdomSubsidenceStepRules.TryBegin(Admitted(), anchor, long.MaxValue,
			...
			ClassicAssert.IsTrue(KingdomSubsidenceStepRules.TryCancel(book, long.MaxValue, 0, out book));
		...
		public void MalformedOrForeignFrozenBooksRefuseWithoutReplacingEvidence(int kind)
		{
			...
			ClassicAssert.IsFalse(KingdomSubsidenceExecutionClockRules.TryValidate(Late, Anchor, book, out string issue));
			StringAssert.Contains("invalid", issue);
			ClassicAssert.AreEqual(option, book?.OptionModel); ClassicAssert.AreSame(active, book?.Active);
		}
		...
		public void FrozenRungPreparationMustNotPostdateActualWorldEvenAtAnAuthorizedClockCut(long now, bool allowed)
		{
			...
			ClassicAssert.IsTrue(KingdomSubsidenceStepRules.TryFreezeRungPlan(book, rung, out book));
			...
			ClassicAssert.AreEqual(KingdomSubsidenceStepPhase.Settling, book.Active.Phase);
			ClassicAssert.AreEqual(Due, book.Active.LastActivityTick);
			ClassicAssert.IsTrue(KingdomSubsidenceStepRules.TryReadRungPlan(book, out rung));
			ClassicAssert.AreEqual(Late, rung.PreparedTick);
		...
		private static KingdomResidentDepartureOperation Departure()
		{
			...
			ClassicAssert.IsTrue(KingdomResidentDepartureRules.Valid(departure)); return departure;
		}
		private static KingdomSubsidenceStepBook Completed()
		{
			...
			ClassicAssert.IsTrue(KingdomSubsidenceStepRules.TryAssociate(book, departure, out book));
			ClassicAssert.IsTrue(KingdomSubsidenceStepRules.TryCredit(book, departure.OperationId, GrowthStage.City, out book));
			ClassicAssert.IsTrue(KingdomSubsidenceStepRules.TryReleaseRetired(book, departure.OperationId, out book)); return book;
		}
		private static KingdomSubsidenceStepBook Freeze(KingdomSubsidenceStepBook book, long before)
		{
			...
			ClassicAssert.IsTrue(KingdomSubsidenceOptionRules.Observe(prior, false, 2, Late,
				out KingdomSubsidenceOptionRules.Snapshot snapshot).Valid);
			ClassicAssert.IsTrue(KingdomSubsidenceOptionIntentRules.TryPrepare(book, before, snapshot, out KingdomSubsidenceOptionIntent intent));
			ClassicAssert.IsTrue(KingdomSubsidenceStepRules.TryFreezeOption(book, intent, out book)); return book;
		}
		private static KingdomSubsidenceStepBook Retired(bool complete, bool frozen)
		{
			KingdomSubsidenceStepBook book = complete ? Completed() : Begin();
			if (frozen) book = Freeze(book, Anchor);
			if (!complete) ClassicAssert.IsTrue(KingdomSubsidenceStepRules.TryCancel(book, Late, 2, out book));
			ClassicAssert.IsTrue(KingdomSubsidenceStepRules.TryRetire(book, complete ? Due : Late, out book)); return book;
		}
		...
		private static void Check(KingdomSubsidenceStepBook book, long now, long raw, bool allowed, string reason = null)
		{
			...
			{
				ClassicAssert.AreEqual(allowed, KingdomSubsidenceExecutionClockRules.TryValidate(now, raw, candidate, out string issue));
				if (allowed) ClassicAssert.IsNull(issue); else ClassicAssert.IsNotEmpty(issue);
				if (reason != null) StringAssert.Contains(reason, issue);
				ClassicAssert.AreEqual(wire, Wire(candidate));
			}
