...
namespace ThousandAndFirst.Tests
{
	...
	public sealed class KingdomCommunalRiteRulesTests
	{
		...
		private static KingdomCommunalRiteBook Bound()
		{
			...
			ClassicAssert.IsTrue(KingdomCommunalRiteRules.TryBindEmptyIdentity(book, Realm(),
		...
		public void ExactAffirmativePracticePreparesCommitsThenEndsAttendedOrSuppressed()
		{
			...
			ClassicAssert.IsTrue(KingdomCommunalRiteRules.TryPracticeSubject(practice.PracticeId,
				out int subject));
			string eventId = KingdomCommunalRiteRules.EventId(practice.SettlementId, 30L, subject);
			ClassicAssert.IsTrue(KingdomCommunalRiteRules.TryPrepare(book, book.Revision, practice,
				eventId, 30L, 2L, out KingdomCommunalRiteReceipt committed,
				out string failure), failure);
			ClassicAssert.AreEqual(KingdomCommunalRitePhase.Prepared, committed.Phase);
			long revision = book.Revision;
			ClassicAssert.IsTrue(KingdomCommunalRiteRules.TryPrepare(book, 0L, practice, eventId,
				30L, 2L, out _, out failure), failure);
			ClassicAssert.AreEqual(revision, book.Revision);
			ClassicAssert.IsFalse(KingdomCommunalRiteRules.TryFinish(book, book.Revision,
				practice.PracticeId, eventId, true, 31L, out _, out _),
				"physical attendance cannot be published from Prepared");
			ClassicAssert.IsFalse(KingdomCommunalRiteRules.TryCommit(book, book.Revision + 1L,
				practice.PracticeId, eventId, out _, out _));
			ClassicAssert.AreEqual(KingdomCommunalRitePhase.Prepared, book.Rows[0].Phase);
			ClassicAssert.IsTrue(KingdomCommunalRiteRules.TryCommit(book, book.Revision,
				practice.PracticeId, eventId, out committed, out failure), failure);
			ClassicAssert.AreEqual(KingdomCommunalRitePhase.Committed, committed.Phase);
			ClassicAssert.IsTrue(KingdomCommunalRiteRules.TryFinish(book, book.Revision,
				practice.PracticeId, eventId, true, 40L, out KingdomCommunalRiteReceipt ended,
				out failure), failure);
			ClassicAssert.AreEqual(KingdomCommunalRitePhase.Attended, ended.Phase);
			ClassicAssert.IsFalse(KingdomCommunalRiteRules.TryFinish(book, book.Revision,
				practice.PracticeId, eventId, false, 41L, out _, out _));
			ClassicAssert.IsTrue(KingdomCommunalRiteRules.TryValidate(book, out failure), failure);
		}
		...
		public void PreparedCancellationIsTerminalAndCannotLaterBecomeAttendance()
		{
			...
			ClassicAssert.AreEqual(KingdomCommunalRitePhase.Suppressed, cancelled.Phase);
			long revision = book.Revision;
			ClassicAssert.IsTrue(KingdomCommunalRiteRules.TryFinish(book, 0L,
		...
		public void ExactReadyRecoveryCanCorrectSuppressionRaceOnlyThroughRecoveryAuthority()
		{
			...
			long revision = book.Revision;
			ClassicAssert.IsFalse(KingdomCommunalRiteRules.TryRecoverReady(book, revision + 1L,
				practice.PracticeId, eventId, 32L, out _, out _));
			ClassicAssert.IsTrue(KingdomCommunalRiteRules.TryRecoverReady(book, revision,
				practice.PracticeId, eventId, 32L,
				out KingdomCommunalRiteReceipt recovered, out failure), failure);
			ClassicAssert.AreEqual(KingdomCommunalRitePhase.Attended, recovered.Phase);
			ClassicAssert.IsTrue(KingdomCommunalRiteRules.TryValidate(book, out failure), failure);
		}
		...
		public void CodecRoundTripsAndCurrentMalformedQuarantinesByteExactly()
		{
			...
			ClassicAssert.AreEqual(book.Revision, restored.Revision);
		...
		public void DeclaredEnvelopeMaximumIsExactAndLeavesExperienceV4Untouched()
		{
			...
			for (int value = 1; found < KingdomCommunalRiteRules.MaxRows && value < 10000; value++)
			{
				...
				ClassicAssert.AreEqual(KingdomCommunalRiteRules.MaxEventIdBytes, eventId.Length);
				ClassicAssert.IsTrue(KingdomCommunalRiteRules.TryPrepare(book, book.Revision, practice,
					eventId, long.MaxValue, long.MaxValue, out _, out string failure), failure);
				found++;
			}
			ClassicAssert.AreEqual(KingdomCommunalRiteRules.MaxRows, found);
			byte[] maximum = KingdomCommunalRiteCodec.EncodeEnvelope(book);
			ClassicAssert.AreEqual(KingdomCommunalRiteCodec.MaxEnvelopeBytes, maximum.Length);
			ClassicAssert.AreEqual(24 * 1024, KingdomExperienceCodec.MaxEnvelopeBytes);
		}
		...
		public void ByteCasRejectsDriftAndAcceptsOneRevisionOnly()
		{
			...
			ClassicAssert.IsTrue(KingdomCommunalRiteRules.TryBindEmptyIdentity(next, Realm(),
				out string failure), failure);
			ClassicAssert.IsFalse(KingdomCommunalRiteCodec.TryPrepareCas(current, "wrong", next,
				out _, out _, out _));
			ClassicAssert.IsTrue(KingdomCommunalRiteCodec.TryPrepareCas(current,
				KingdomCommunalRiteCodec.DigestHex(current), next, out byte[] replacement,
				out string digest, out failure), failure);
			ClassicAssert.AreEqual(digest, KingdomCommunalRiteCodec.DigestHex(replacement));
			...
			ClassicAssert.IsFalse(KingdomCommunalRiteCodec.TryPrepareCas(last,
		...
		public void CharterRiteLeavesBothW0CollectionsByteIdentical()
		{
			...
			ClassicAssert.IsTrue(KingdomExperienceRules.TryBindEmptyIdentity(w0, Realm(),
				out string failure), failure);
			...
			string eventId = KingdomCommunalRiteRules.EventId(practice.SettlementId, 30L,
				subject);
			ClassicAssert.IsTrue(KingdomCommunalRiteRules.TryPrepare(rites, rites.Revision, practice,
				eventId, 30L, 1L, out _, out failure), failure);
			ClassicAssert.IsTrue(KingdomCommunalRiteRules.TryCommit(rites, rites.Revision,
				practice.PracticeId, eventId, out _, out failure), failure);
			ClassicAssert.IsTrue(KingdomCommunalRiteRules.TryFinish(rites, rites.Revision,
				practice.PracticeId, eventId, true, 31L, out _, out failure), failure);
			ClassicAssert.AreEqual(0, w0.Audiences.Count);
			ClassicAssert.AreEqual(0, w0.BodyReservations.Count);
		...
		{
			ClassicAssert.AreEqual(KingdomCommunalRiteOptionDisposition.Unreadable,
				KingdomCommunalRiteRules.OptionDisposition(false, true, 2L, 2L));
			ClassicAssert.AreEqual(KingdomCommunalRiteOptionDisposition.Disabled,
				KingdomCommunalRiteRules.OptionDisposition(true, false, 0L, 2L));
			ClassicAssert.AreEqual(KingdomCommunalRiteOptionDisposition.Unreadable,
				KingdomCommunalRiteRules.OptionDisposition(true, true, 0L, 2L));
			ClassicAssert.AreEqual(KingdomCommunalRiteOptionDisposition.Current,
				KingdomCommunalRiteRules.OptionDisposition(true, true, 2L, 2L));
			ClassicAssert.AreEqual(KingdomCommunalRiteOptionDisposition.SupersededEpoch,
