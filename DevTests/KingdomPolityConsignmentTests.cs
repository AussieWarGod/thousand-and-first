...
namespace ThousandAndFirst.DevTests
{
	...
	public sealed class KingdomPolityConsignmentTests
	{
		...
		public void PartialReceiptCreditsExactQuantityAndRelationOnce()
		{
			...
			ClassicAssert.AreEqual(8, request.RequestedDrams);
			ClassicAssert.AreEqual(KingdomPolityTestData.Rival, request.CounterpartyPolityId);
			...
			ClassicAssert.IsTrue(KingdomPolityCorrespondenceRules.TryPlanConsignment(ledger, 0L,
			...
			ClassicAssert.AreEqual(KingdomPolityCorrespondenceReplyKind.Fulfilled, reply);
			KingdomPolityIncidentRecord plan = Incident(ledger, request.CorrespondencePlanId);
			ClassicAssert.IsTrue(HasPrefix(plan.Conclusion.ReceiptRefs,
				"taf:receipt:trade-operation-proof:v1:"));
			ClassicAssert.IsFalse(plan.Conclusion.ReceiptRefs.Contains(receipt.TradeOperationId));
			ClassicAssert.AreEqual(1, plan.Conclusion.SystemicDeltas.Count);
			ClassicAssert.AreEqual(KingdomPolitySystemicDeltaKind.Standing,
				plan.Conclusion.SystemicDeltas[0].Kind);
			ClassicAssert.AreEqual(7, plan.Conclusion.SystemicDeltas[0].Amount);
			ClassicAssert.AreEqual(1, plan.Conclusion.RelationDeltas.Count);
			ClassicAssert.AreEqual(KingdomPolityRelationBand.Neutral, Relation(ledger).Band);
		...
		public void ZeroOrReplacementCannotCreditAndDeclineCannotFarmRelation()
		{
			...
			ClassicAssert.IsFalse(KingdomPolityCorrespondenceRules.TryConsumeTradeReceipt(ledger,
			...
			ClassicAssert.IsTrue(KingdomPolityCorrespondenceRules.TryDeclineConsignment(ledger,
			...
			ClassicAssert.AreEqual(before, Relation(ledger).Band);
			ClassicAssert.AreEqual(0, Incident(ledger,
		...
		public void TerminalFailureBecomesOneZeroDeltaUnfulfilledReply()
		{
			...
			ClassicAssert.IsTrue(KingdomPolityCorrespondenceRules.TryDescribeConsignment(ledger,
				request.CorrespondencePlanId, out _,
				out KingdomPolityCorrespondenceReplyKind reply, out failure), failure);
			ClassicAssert.AreEqual(KingdomPolityCorrespondenceReplyKind.Unfulfilled, reply);
			KingdomPolityIncidentConclusion conclusion = Incident(ledger,
				request.CorrespondencePlanId).Conclusion;
			ClassicAssert.AreEqual(0, conclusion.SystemicDeltas.Count);
			ClassicAssert.AreEqual(0, conclusion.RelationDeltas.Count);
			ClassicAssert.AreEqual(band, Relation(ledger).Band);
		...
		public void HistoricalRequestsRemainReadableAcrossEveryReplyAfterRealmEnding()
		{
			...
			for (int i = 0; i < replies.Length; i++)
			{
				...
				ClassicAssert.AreEqual(request.RequestDigest, retried.RequestDigest);
				ClassicAssert.IsTrue(KingdomPolityCorrespondenceRules.TryDescribeConsignment(ledger,
				...
				ClassicAssert.AreEqual(request.CurrentPolityId, restored.CurrentPolityId);
				ClassicAssert.AreEqual(request.CounterpartyPolityId, restored.CounterpartyPolityId);
				ClassicAssert.AreEqual(replies[i], actual);
				KingdomPolityLedger decoded = KingdomPolityCodec.DecodeEnvelope(
					KingdomPolityCodec.EncodeEnvelope(ledger));
				ClassicAssert.IsTrue(KingdomPolityCorrespondenceRules.TryDescribeConsignment(decoded,
		...
		public void TradeReaderClassifiesMissingPartialFailureAndInvalidExactly()
		{
			...
			ClassicAssert.AreEqual(7, receipt.DeliveredDrams);
			ClassicAssert.IsTrue(KingdomPolityRules.Digest(receipt.TradeOperationId));
			ClassicAssert.AreEqual(RecipientBodyId(), receipt.RecipientBodyId);
			book.RecentProofs[0].PolityRecipient = null;
			ClassicAssert.IsTrue(KingdomTradeRules.TryInspectPolityConsignmentReceipt(book, request,
				out _, out kind, out failure));
			ClassicAssert.AreEqual(KingdomTradePolityConsignmentReceiptKind.Invalid, kind);
		...
		public void AvailableRouteWithoutLawfulDepartureReceiptIsRejected()
		{
			...
			ClassicAssert.IsFalse(KingdomPolityRules.TryValidate(ledger, out string failure));
