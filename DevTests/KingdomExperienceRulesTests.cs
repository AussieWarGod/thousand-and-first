...
namespace ThousandAndFirst.Tests
{
	...
	public sealed class KingdomExperienceRulesTests
	{
		...
		private static KingdomExperienceLedger Enabled(long tick = 10L)
		{
			...
			ClassicAssert.IsTrue(KingdomExperienceRules.TryBindEmptyIdentity(l, Realm,
				out string failure), failure);
			ClassicAssert.IsTrue(KingdomExperienceRules.TryObserveOptions(l, l.Revision,
		...
		{
			ClassicAssert.AreEqual(3, KingdomExperienceRules.MaxSettlements);
			ClassicAssert.AreEqual(1, KingdomExperienceRules.MaxAudienceReceipts /
				KingdomExperienceRules.MaxSettlements);
			ClassicAssert.AreEqual(16, KingdomExperienceRules.MaxTransientBodySlots);
			ClassicAssert.AreEqual(16, KingdomExperienceRules.MaxBodyReservations);
			ClassicAssert.AreEqual(7, KingdomExperienceRules.MaxBodiesPerReservation);
			ClassicAssert.Less(KingdomExperienceRules.MaxDeclaredPayloadBytes,
		...
		{
			ClassicAssert.AreEqual(KingdomLoadReconciliationMode.None,
				KingdomLoadReconciliationRules.Select(false, false));
			ClassicAssert.AreEqual(KingdomLoadReconciliationMode.None,
				KingdomLoadReconciliationRules.Select(false, true));
			ClassicAssert.AreEqual(KingdomLoadReconciliationMode.CommittedCapacityOnly,
				KingdomLoadReconciliationRules.Select(true, false));
			ClassicAssert.AreEqual(KingdomLoadReconciliationMode.Full,
		...
		public void MasterPublicationGateRefusesEveryBoundaryBeforeAnyOwnerChanges()
		{
			...
			for (int injected = 0; injected < count; injected++)
			{
				...
				ClassicAssert.IsFalse(KingdomMasterPublicationGate.TryOpen(exact, count,
					injected, out string _));
				AssertOwners(before, owners);
			}
			ClassicAssert.IsTrue(KingdomMasterPublicationGate.TryOpen(exact, count, -1,
				out string failure), failure);
		...
		public void SharedBodyUnionProtectsQueuedDeliveriesWithoutDoubleCountingBindings()
		{
			...
			ClassicAssert.IsTrue(KingdomSharedBodyCapacityRules.TryCountFoundationClaims(bindings,
				deliveries, out int count, out KingdomExperienceCapacityFault fault,
				out string failure), failure);
			ClassicAssert.AreEqual(4, count);
			ClassicAssert.IsTrue(KingdomSharedBodyCapacityRules.TryAdmitFoundationClaims(bindings,
				deliveries, new int[] { 4 }, 12, out count, out fault, out failure), failure);
			ClassicAssert.AreEqual(4, count, "binding a queued delivery must consume its existing claim");
			ClassicAssert.IsFalse(KingdomSharedBodyCapacityRules.TryAdmitFoundationClaims(bindings,
			...
			ClassicAssert.IsFalse(KingdomSharedBodyCapacityRules.TryAdmitNewFoundationClaims(bindings,
				deliveries, 1, 12, out count, out fault, out failure));
			ClassicAssert.AreEqual("CapacityFull(foundation-bodies:realm)", failure);
			...
			ClassicAssert.IsFalse(KingdomExperienceRules.TryReserveBodies(ledger, ledger.Revision,
			...
			ClassicAssert.IsTrue(KingdomSharedBodyCapacityRules.TryCountFoundationClaims(
				legacyBindings, legacyDeliveries, out count, out fault, out failure), failure);
			ClassicAssert.AreEqual(32, count);
			ClassicAssert.IsTrue(KingdomSharedBodyCapacityRules.TryAdmitFoundationClaims(
				legacyBindings, legacyDeliveries, new int[] { 32 }, 0, out count,
				out fault, out failure), failure);
			ClassicAssert.IsFalse(KingdomSharedBodyCapacityRules.TryAdmitFoundationClaims(
				legacyBindings, legacyDeliveries, new int[] { 33 }, 0, out count,
				out fault, out failure));
			ClassicAssert.AreEqual(KingdomExperienceCapacityFault.LiveBodyCapacityFull, fault);
			ClassicAssert.IsFalse(KingdomSharedBodyCapacityRules.TryAdmitNewFoundationClaims(
			...
			int protectedFoundationBodies)
		{
			...
			ClassicAssert.IsTrue(KingdomExperienceRules.TryReservePresentation(ledger,
			...
			ClassicAssert.IsTrue(KingdomExperienceRules.TryRecoverRetirementBodies(ledger,
				ledger.Revision, bodies, protectedFoundationBodies, out fault, out failure), failure);
			ClassicAssert.IsTrue(KingdomExperienceRules.TryRecoverRetirementPresentation(ledger,
				ledger.Revision, audience, bodies, protectedFoundationBodies,
				out fault, out failure), failure);
			CollectionAssert.AreEqual(retirement, KingdomExperienceCodec.EncodeEnvelope(ledger));
			ClassicAssert.IsFalse(KingdomExperienceRules.TryRecoverDurableBodies(ledger,
				ledger.Revision, Body("legacy-overcap-missing"), protectedFoundationBodies,
				out fault, out failure));
			ClassicAssert.AreEqual(KingdomExperienceCapacityFault.LiveBodyCapacityFull, fault);
			CollectionAssert.AreEqual(retirement, KingdomExperienceCodec.EncodeEnvelope(ledger));
			ClassicAssert.IsTrue(KingdomExperienceRules.TryReleasePresentation(ledger,
				ledger.Revision, audience.ReservationId, bodies.ReservationId, source,
				out fault, out failure), failure);
			ClassicAssert.AreEqual(0, ledger.Audiences.Count);
			ClassicAssert.AreEqual(0, ledger.BodyReservations.Count);
		}
		...
		public void MasterResumeReanchorsEveryConfiguredEpochWithoutReplayingPausedCauses()
		{
			...
			ClassicAssert.IsTrue(KingdomExperienceRules.TryPublishMasterResume(ledger, plan,
				out failure), failure);
			ClassicAssert.AreEqual(2L, ledger.Story.EnableEpoch);
			ClassicAssert.AreEqual(30L, ledger.Story.FutureCauseFloorTick);
			ClassicAssert.AreEqual(KingdomExperienceOptionState.Disabled, ledger.Knowledge.State);
			ClassicAssert.IsFalse(KingdomExperienceRules.CanEmit(ledger,
				KingdomExperienceOptionKind.CivicStory, 29L));
			ClassicAssert.IsTrue(KingdomExperienceRules.CanEmit(ledger,
				KingdomExperienceOptionKind.CivicStory, 30L));
			ClassicAssert.IsTrue(KingdomExperienceRules.TryReadBodyLease(ledger,
			...
			ClassicAssert.IsFalse(KingdomExperienceRules.TryPrepareMasterResume(impossible, Realm,
			...
			ClassicAssert.IsTrue(KingdomExperienceRules.TryPrepareMasterResume(equalTick, Realm,
				20L, 20L, true, true, true, out plan, out failure), failure);
			ClassicAssert.IsTrue(KingdomExperienceRules.TryPublishMasterResume(equalTick, plan,
				out failure), failure);
			ClassicAssert.IsTrue(KingdomExperienceRules.TryReadBodyLease(equalTick,
			...
			ClassicAssert.IsTrue(KingdomExperienceRules.TryPrepareMasterResume(divergent, Realm,
			...
			ClassicAssert.AreEqual(sameRevision, divergent.Revision);
			ClassicAssert.IsTrue(KingdomExperienceRules.TryValidate(divergent, out failure), failure);
			ClassicAssert.IsFalse(KingdomExperienceRules.TryPublishMasterResume(divergent, plan,
				out failure));
		...
		public void DurableRecoveryRepairsExactProjectedProofInActiveOrRetirementEpoch()
		{
			...
			ClassicAssert.IsTrue(KingdomExperienceRules.TryClassifyLeaseProof(ledger,
				request.OptionKind, request.CauseTick, request.ReservedTick,
				request.EnableEpoch, out KingdomExperienceLeaseState proofState,
				out string failure), failure);
			ClassicAssert.AreEqual(KingdomExperienceLeaseState.Active, proofState);
			ClassicAssert.IsFalse(KingdomExperienceRules.TryRecoverRetirementBodies(ledger,
				ledger.Revision, request, 2, out KingdomExperienceCapacityFault _,
				out string _));
			ClassicAssert.IsTrue(KingdomExperienceRules.TryRecoverDurableBodies(ledger,
				ledger.Revision, request, 2, out _, out failure), failure);
			ClassicAssert.IsTrue(KingdomExperienceRules.TryReleaseBodies(ledger, ledger.Revision,
			...
			ClassicAssert.AreEqual(KingdomExperienceLeaseState.Retirement, proofState);
			ClassicAssert.IsTrue(KingdomExperienceRules.TryRecoverDurableBodies(ledger,
				ledger.Revision, request, 2, out _, out failure), failure);
			ClassicAssert.IsTrue(KingdomExperienceRules.TryReadBodyLease(ledger, request.ReservationId,
		...
		public void BlankAndBoundLedgersRoundTripCanonically()
		{
			...
			ClassicAssert.IsTrue(KingdomExperienceRules.TryValidate(blank, out string failure), failure);
			...
			ClassicAssert.AreEqual(Realm, read.RealmId);
			ClassicAssert.AreEqual(KingdomExperienceOptionState.Enabled, read.Story.State);
		...
		public void OptionsDisableWithoutBacklogAndRetainSourceOwnedCapacityLeases()
		{
			...
			ClassicAssert.IsFalse(KingdomExperienceRules.TryObserveOptions(premature,
			...
			ClassicAssert.IsFalse(KingdomExperienceRules.CanEmit(l,
				KingdomExperienceOptionKind.CivicStory, 99L));
			ClassicAssert.IsTrue(KingdomExperienceRules.CanEmit(l,
				KingdomExperienceOptionKind.CivicStory, 100L));
			ClassicAssert.IsTrue(KingdomExperienceRules.TryGetEnableEpoch(l,
				KingdomExperienceOptionKind.CivicStory, 100L, out long epoch,
				out string epochFailure), epochFailure);
			ClassicAssert.AreEqual(1L, epoch);
			Reserve(l, Audience("old", cause: 100L, reserved: 100L));
			Reserve(l, Body("old", cause: 100L, reserved: 100L), 0);
			ClassicAssert.AreEqual(1, l.Audiences.Count); ClassicAssert.AreEqual(1, l.BodyReservations.Count);
			...
			ClassicAssert.IsFalse(KingdomExperienceRules.TryReserveBodies(l, l.Revision,
			...
			ClassicAssert.AreEqual(KingdomExperienceCapacityFault.OptionDisabled, disabledFault);
			...
			ClassicAssert.AreEqual(2L, epoch);
			byte[] beforeStaleRetry = KingdomExperienceCodec.EncodeEnvelope(l);
			ClassicAssert.IsFalse(KingdomExperienceRules.TryReserveAudience(l, l.Revision,
				Audience("old", cause: 100L, reserved: 100L, epoch: 1L),
				out KingdomExperienceCapacityFault fault, out failure));
			ClassicAssert.AreEqual(KingdomExperienceCapacityFault.CauseBeforeEnable, fault);
			CollectionAssert.AreEqual(beforeStaleRetry, KingdomExperienceCodec.EncodeEnvelope(l));
			ClassicAssert.IsTrue(KingdomExperienceRules.TryReleaseAudience(l, l.Revision,
				"taf:experience-audience:old", "taf:event:old", out fault, out failure), failure);
			ClassicAssert.IsTrue(KingdomExperienceRules.TryReleaseBodies(l, l.Revision,
				"taf:experience-body:old", "taf:event:old", out fault, out failure), failure);
			ClassicAssert.AreEqual(0, l.Audiences.Count); ClassicAssert.AreEqual(0, l.BodyReservations.Count);
		...
		public void RetirementLeasesRemainInTheSharedCapAcrossDisableAndReenable()
		{
			...
			ClassicAssert.AreEqual(16, KingdomExperienceRules.ReservedBodies(l));
		...
		public void OptionDisableBeforeAReservedTickRefusesByteStably()
		{
			...
			ClassicAssert.IsFalse(KingdomExperienceRules.TryObserveOptions(l, l.Revision,
		...
		public void ExactRetirementRecoveryIsCapacityOnlyAtomicAndClassified()
		{
			...
			KingdomExperienceBodyReservation bodies = Body("retirement-recovery", 3,
				lane: KingdomExperienceLane.PolityCohort,
				option: KingdomExperienceOptionKind.AmbientUse, source: source);
			ClassicAssert.IsTrue(KingdomExperienceRules.TryRecoverRetirementPresentation(l,
				l.Revision, audience, bodies, 4, out KingdomExperienceCapacityFault fault,
				out failure), failure);
			ClassicAssert.IsTrue(KingdomExperienceRules.TryReadAudienceLease(l,
			...
			ClassicAssert.AreEqual(KingdomExperienceLeaseState.Retirement, audienceState);
			ClassicAssert.AreEqual(source, readAudience.SourceId);
			readAudience.SourceId = "taf:event:mutated-copy";
			ClassicAssert.AreEqual(source, l.Audiences[0].SourceId);
			ClassicAssert.IsTrue(KingdomExperienceRules.TryReadBodyLease(l, bodies.ReservationId,
				out KingdomExperienceBodyReservation readBodies,
				out KingdomExperienceLeaseState bodyState, out failure), failure);
			ClassicAssert.AreEqual(KingdomExperienceLeaseState.Retirement, bodyState);
			ClassicAssert.AreEqual(3, readBodies.BodyCount);
			long stable = l.Revision;
			ClassicAssert.IsTrue(KingdomExperienceRules.TryRecoverRetirementPresentation(l, 0L,
				audience, bodies, 4, out fault, out failure), failure);
			ClassicAssert.AreEqual(stable, l.Revision);
			ClassicAssert.IsFalse(KingdomExperienceRules.TryReservePresentation(l, l.Revision,
		...
		public void RealmRemovalBlocksEveryLiveW0LeaseAndRefusesUnknownOwnership()
		{
			...
			ClassicAssert.IsTrue(KingdomExperienceRules.TryDescribeRealmRemovalBlocker(empty, Realm,
			...
			ClassicAssert.IsTrue(KingdomExperienceRules.TryDescribeRealmRemovalBlocker(live, Realm,
			...
			ClassicAssert.IsFalse(KingdomExperienceRules.TryDescribeRealmRemovalBlocker(live,
				"taf:realm:other", out blocker, out failure));
			StringAssert.Contains("another realm", failure);
			ClassicAssert.IsFalse(KingdomExperienceRules.TryDescribeRealmRemovalBlocker(null, Realm,
		...
		public void RetirementPresentationCapFailureCreatesNoPartialAudience()
		{
			...
			byte[] before = KingdomExperienceCodec.EncodeEnvelope(l);
			ClassicAssert.IsFalse(KingdomExperienceRules.TryRecoverRetirementPresentation(l,
				l.Revision, audience, bodies, 0, out KingdomExperienceCapacityFault fault,
				out failure));
			ClassicAssert.AreEqual(KingdomExperienceCapacityFault.LiveBodyCapacityFull, fault);
			ClassicAssert.AreEqual(0, l.Audiences.Count);
			CollectionAssert.AreEqual(before, KingdomExperienceCodec.EncodeEnvelope(l));
		}
		...
		public void OneAudiencePerSettlementNeverEvictsOrMutatesOnFailure()
		{
			...
			ClassicAssert.AreEqual(3, l.Audiences.Count);
		}
		...
		public void IdenticalRetryIsNoOpAndIdentityMismatchFailsClosed()
		{
			...
			ClassicAssert.Greater(published, before);
			ClassicAssert.IsTrue(KingdomExperienceRules.TryReserveAudience(l, before, request,
				out KingdomExperienceCapacityFault fault, out string failure), failure);
			ClassicAssert.AreEqual(published, l.Revision);
			KingdomExperienceAudienceReceipt mismatch = Audience("same");
			mismatch.SourceId = "taf:event:different";
			ClassicAssert.IsFalse(KingdomExperienceRules.TryReserveAudience(l, l.Revision, mismatch,
				out fault, out failure));
			ClassicAssert.AreEqual(KingdomExperienceCapacityFault.DuplicateMismatch, fault);
		}
		...
		public void BodyLeaseSharesSixteenSlotLiveBindingCeiling()
		{
			KingdomExperienceLedger l = Enabled();
			byte[] before = KingdomExperienceCodec.EncodeEnvelope(l);
			ClassicAssert.IsFalse(KingdomExperienceRules.TryReserveBodies(l, l.Revision, Body("seven", 7),
				10, out KingdomExperienceCapacityFault fault, out string failure));
			ClassicAssert.AreEqual(KingdomExperienceCapacityFault.LiveBodyCapacityFull, fault);
			ClassicAssert.AreEqual("CapacityFull(live-bodies:realm)", failure);
			CollectionAssert.AreEqual(before, KingdomExperienceCodec.EncodeEnvelope(l));

			Reserve(l, Body("six", 6), 10);
			ClassicAssert.AreEqual(6, KingdomExperienceRules.ReservedBodies(l));
			ClassicAssert.IsFalse(KingdomExperienceRules.TryReserveBodies(l, l.Revision, Body("one", 1),
		...
		public void PolityPresentationAtomicallySharesAudienceAndSixteenBodyAuthority()
		{
			...
			ClassicAssert.AreEqual(1, l.Audiences.Count);
			ClassicAssert.AreEqual(16, KingdomExperienceRules.ReservedBodies(l));
			byte[] atCap = KingdomExperienceCodec.EncodeEnvelope(l);
		...
		public void DirectedBodyBypassSkipsAudienceCompetitionButNeverBodyCapacity()
		{
			...
			ClassicAssert.AreEqual(KingdomExperienceCapacityFault.LiveBodyCapacityFull, fault);
			CollectionAssert.AreEqual(before, KingdomExperienceCodec.EncodeEnvelope(l));
			ClassicAssert.IsFalse(KingdomExperienceRules.TryReleaseAudience(l, l.Revision,
		...
		public void PolityPresentationRoundTripsAndOnlyItsExactCohortCanReleaseIt()
		{
			...
			ClassicAssert.AreEqual(KingdomExperienceLane.PolityCohort,
				recovered.Audiences[0].Lane);
			ClassicAssert.AreEqual(KingdomExperienceLane.PolityCohort,
				recovered.BodyReservations[0].Lane);
			long stableRevision = recovered.Revision;
			ClassicAssert.IsTrue(KingdomExperienceRules.TryReservePresentation(recovered, 0L,
				audience, polity, 0, out KingdomExperienceCapacityFault retryFault,
				out string retryFailure), retryFailure);
			ClassicAssert.AreEqual(KingdomExperienceCapacityFault.None, retryFault);
			ClassicAssert.AreEqual(stableRevision, recovered.Revision);
			ClassicAssert.IsFalse(KingdomExperienceRules.TryReleasePresentation(recovered,
				recovered.Revision, audience.ReservationId, polity.ReservationId,
				"taf:cohort:foreign",
				out KingdomExperienceCapacityFault fault, out string failure));
			ClassicAssert.AreEqual(KingdomExperienceCapacityFault.OwnershipMismatch, fault);
			ClassicAssert.IsTrue(KingdomExperienceRules.TryReleasePresentation(recovered,
				recovered.Revision, audience.ReservationId, polity.ReservationId,
				polity.SourceId, out fault, out failure), failure);
			ClassicAssert.AreEqual(0, recovered.Audiences.Count);
			ClassicAssert.AreEqual(0, KingdomExperienceRules.ReservedBodies(recovered));
		}
		...
		public void ReservationReleaseIsOwnedAndIdempotent()
		{
			...
			byte[] before = KingdomExperienceCodec.EncodeEnvelope(l);
			ClassicAssert.IsFalse(KingdomExperienceRules.TryReleaseBodies(l, l.Revision,
			...
			ClassicAssert.IsTrue(KingdomExperienceRules.TryReleaseBodies(l, 0L,
				"taf:experience-body:owned", "taf:event:owned", out fault, out failure), failure);
			ClassicAssert.AreEqual(closed, l.Revision);
		}
		...
		public void ActiveLeasesPreventImplicitRealmRebinding()
		{
			...
			byte[] before = KingdomExperienceCodec.EncodeEnvelope(l);
			ClassicAssert.IsFalse(KingdomExperienceRules.TryRebindEmptyIdentity(l,
		...
		public void CrossRealmRequestFailsWithoutChangingAuthority()
		{
			...
			byte[] before = KingdomExperienceCodec.EncodeEnvelope(l);
			ClassicAssert.IsFalse(KingdomExperienceRules.TryReserveAudience(l, l.Revision, request,
				out KingdomExperienceCapacityFault fault, out string failure));
			ClassicAssert.AreEqual(KingdomExperienceCapacityFault.WrongRealm, fault);
			StringAssert.Contains("another realm", failure);
			CollectionAssert.AreEqual(before, KingdomExperienceCodec.EncodeEnvelope(l));
		}
		...
		public void FutureWireIsBoundedOpaqueAndRoundTripsExactly()
		{
			...
			ClassicAssert.AreEqual(KingdomExperienceSchemaState.Unknown, read.SchemaState);
			ClassicAssert.IsFalse(KingdomExperienceRules.CanEmit(read,
				KingdomExperienceOptionKind.CivicStory, long.MaxValue));
		...
		public void MaximumLegalRowsRemainInsideDeclaredEnvelope()
		{
			...
			ClassicAssert.IsTrue(KingdomExperienceRules.TryValidate(l, out string failure), failure);
		}

		private static void Reserve(KingdomExperienceLedger L,
			KingdomExperienceAudienceReceipt R)
		{
			ClassicAssert.IsTrue(KingdomExperienceRules.TryReserveAudience(L, L.Revision, R,
		...
		{
			ClassicAssert.IsTrue(KingdomExperienceRules.TryReserveBodies(L, L.Revision, R, live,
				out KingdomExperienceCapacityFault _, out string failure), failure);
		}
		...
		{
			ClassicAssert.IsTrue(KingdomExperienceRules.TryReservePresentation(L, L.Revision, A, B, live,
				out KingdomExperienceCapacityFault _, out string failure), failure);
		}
