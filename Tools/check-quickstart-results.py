...
_SPEC = importlib.util.spec_from_file_location("quickstart_safe_files", TOOLS / "prepare-scenario-load.py")
...
OBSERVED = "actual founded heart, finite single grants and advisor verified after GAMESTARTING"
SAVE_COMPLETE = ("real-save=true; exact-owner-and-stock=true; no-bootstrap-replay=true; "
...
LOAD_COMPLETE = ("real-save-owned-stop-cold-load=true; graceful-quit=false; unchanged-heart-stock-and-IDs=true"
                 "; bootstrap-replay=false; ordinary-acceptance=false")
AUTOSTART = ("AUTOSTART", "OK", "sealed script present; test game started without input")
...
def journal(raw: bytes, phase: str, command: str, seed: str, game_id: str | None) -> bool:
    ...
    for line in lines:
        ...
        require(re.fullmatch(r"[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]{3}Z", stamp),
    ...
    if phase == "save":
        expected += [("QUICKSTART-SAVE-BEGIN", "genuine-world=true; game-id=" + game_id + "; synthetic-state=false"),
...
def snapshot_identity(raw: bytes) -> tuple:
    ...
    require(len(wire) <= 2097152 and wire.startswith(SNAPSHOT_PREFIX), "not a bounded Quickstart snapshot")
    payload = wire[len(SNAPSHOT_PREFIX):]
    data = base64.b64decode(payload, validate=True)
    require(base64.b64encode(data).decode("ascii") == payload, "noncanonical snapshot base64")
    at = 0
    ...
    def text(maximum, optional=False):
        ...
        if length == -1 and optional:
            return None
        require(0 <= length <= maximum * 4 and at + length <= len(data), "snapshot text length refused")
        value = data[at:at + length].decode("utf-8")
        at += length
        require((value or optional) and len(value.encode("utf-16-le")) // 2 <= maximum
                and not any(ord(c) < 32 or 127 <= ord(c) <= 159 for c in value), "snapshot text refused")
        return value

    require(number("<i") == 0x31535154 and number("<i") == 1, "snapshot magic/version differs")
    game_id, seed, selected = text(36), text(97), text(6)
    advisor = number("B")
    require(files.GUID.fullmatch(game_id) and advisor in (0, 1) and number("<i") > 0, "snapshot identity refused")
...
def verify_log(raw: bytes) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="taf-quickstart-log.") as scratch:
        ...
        checked = subprocess.run(["bash", str(TOOLS / "check-player-log.sh"), str(path)], env=environment,
...
def verify(root: Path, phase: str) -> dict:
    ...
    embark = read(local / "Mods/ThousandAndFirst/Harness/EmbarkModules.xml", files.MAX_LOCAL_FILE).decode("utf-8")
    ...
    if phase not in ("boot", "build"):
        ...
        game_id = receipt[1]
        snapshot_path = local / "scenario-load-snapshot.txt" if phase == "load" else root / "scenario-save-snapshot.txt"
        raw = read(snapshot_path, 2097152)
        require(hashlib.sha256(raw).hexdigest() == receipt[-2], "snapshot hash mismatch")
        require(snapshot_identity(raw) == (game_id, seed, tokens[1], tokens[2]), "snapshot/selection identity mismatch")
        ...
        require(sorted(p.name for p in save.iterdir()) == sorted(files.SAVE_FILES), "save artifact inventory differs")
        ...
        require(hashes[files.SAVE_FILES[0]][0] == receipt[2] and hashes[files.SAVE_FILES[1]][0] == receipt[3],
                "primary/info hash mismatch")
        require(phase != "load" or hashes["Cache.db"][0] == receipt[4], "sealed cache hash mismatch")
    ...
    for path, (maximum, before, stamp) in frozen.items():
        require(files.read_bytes(path, maximum) == before and files.stamp(files.file_status(path, maximum)) == stamp,
                "evidence changed during verdict: " + path.name)
    if save is not None:
        require(sorted(p.name for p in save.parent.iterdir()) == [game_id]
                and sorted(p.name for p in save.iterdir()) == sorted(files.SAVE_FILES), "save inventory changed")
    ...
    return {"verdict": "PASS", "phase": phase, "command": command, "seed": seed, "gameId": game_id,
            "journalSHA256": hashlib.sha256(journal_raw).hexdigest(), "playerLogSHA256": hashlib.sha256(log_raw).hexdigest(),
            "saveHashes": {name: digest for name, (digest, _) in hashes.items()},
            "scope": "sealed-developer-quickstart-evidence", "snapshotSemantics": "nested-native-witness-not-reproved",
            "autostartObserved": autostart, "cacheReceiptBound": phase == "load", "processAuthority": False, "ordinaryAcceptance": False,
            "historicalSaveCompatibility": False, "releaseAcceptance": False}
...
def main(argv: list[str]) -> int:
    try:
        ...
        print(json.dumps({"verdict": "REFUSED", "reason": str(error), "ordinaryAcceptance": False,
                          "releaseAcceptance": False}, ensure_ascii=True))
        return 2
