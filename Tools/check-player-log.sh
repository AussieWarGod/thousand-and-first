...
# Match the complete manifest title, including the optional development-profile suffix;
# a similarly named mod such as The Thousand and First Helper is not this title.
...
awk -v allow="${TAF_LOG_ALLOW:-}" '
	{
		...
		lower = tolower($0)
		if ($0 ~ /^MOD(ERROR|WARN) \[The Thousand and First( \[ALPHA\])?( \[DEV SCENARIO HARNESS\])?\]([[:space:]]|$)/ ||
