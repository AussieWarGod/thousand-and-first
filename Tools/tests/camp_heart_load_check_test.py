...
class CampHeartLoadCheckTests(unittest.TestCase):
    def fixture(self):
        ...
        source += [('camp-heart-save', 'OK', 'paid-camp-save=true; rung=2; synthetic-next-job-timber=1; brush=23; save=' + game + '; ' + physical + '; ' + digest),
                   ('SCRIPT-COMPLETE', 'OK', 'done')]
        loaded = [('LOAD-BEGIN', 'OK', 'exact sealed save; game-id=' + game + '; new-game=false; mod-restore=false'),
                  ('camp-heart-loaded', 'OK', 'rung=2; basin=48; brush=23; timber=1; ' + physical + '; ' + digest),
                  ...
                  ('camp-heart-completed', 'OK', 'new-job=next; phase=Complete; effects-settled=true; output=new-fire; ' + physical + '; brush=23; turns=9602'),
    ...
    def test_paid_loaded_job_really_completed(self):
        ...
        self.assertEqual(('PASS', 'next', 'new-fire', 6000, 3600, 23),
    ...
    def test_loaded_identity_payment_completion_and_resume_drift_fail(self):
        changes = ((0, 'game-id=01234567', 'game-id=11234567'), (0, 'new-game=false', 'new-game=true'),
                   ...
                   (1, 'brush=23', 'brush=22'), (1, 'a' * 64, 'b' * 64),
                   ...
                   (5, 'brush=23', 'brush=22'), (6, 'new-game-script-replayed=false', 'new-game-script-replayed=true'))
    ...
    def test_replay_duplicate_fields_and_rows_after_terminal_fail(self):
        ...
        loaded[1] = event, status, detail + '; brush=23'
        with self.assertRaises(ValueError): check.judge(source, loaded)
        source, loaded = self.fixture()
