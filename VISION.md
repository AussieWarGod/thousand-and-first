...
*Where this mod is going, and the rules it holds itself to.*

This is direction, not a schedule. Nothing here is a promise about when — or whether — a
given piece ships. It is written down so that anyone who wants to build on the mod knows what
it is trying to be, and can tell whether their idea belongs.
...
**It is not a second job.** The settlement never fails on a timer, never nags, and never
punishes you for being away. Every system is a bonus for engaging, never a penalty for
abstaining, and every module can be switched off. A player who never founds anything should
...
flatly denies. History in Qud is contested text, and yours should be too.
...
The mod is designed to be a platform, not a monolith. Buildings, city styles, and trade
charters are **data**: ship an XML file with the right root element from your own mod and it
extends the registries with no code and no dependency declaration. Building blueprints are
ordinary Qud objects, so anything you already know how to make can become a commissionable
work, and anything with vanilla parts is used by the settlement for free.
