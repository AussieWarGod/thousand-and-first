...
| Staged runtime inventory | **PASS — FOUR CANONICAL COMPILE MODES, 2026-09-05 02:27:31 UTC** | Current resolver selects 2950 sources, baseline and compatibility symbols. Ordinary baseline/compatibility compile 2946/2950 sources; development-harness baseline/compatibility compile 2992/2996, including 46 harness shards. Current 2981-file stage, installed Hearthpyre 2.2.3 hashes/ABI, generators, and exact compile receipt pass. Retained `/tmp/taf-stage.J3oh60` and `/tmp/taf-devharness.358jyu`. Gate uses tracked `latest` language setting; strict 7.3 result below is separate. None of this is native compatibility behavior. |
| Strict C# 7.3 direct-source preflight | **FAIL — pre-existing CS0136 in all four modes** | Explicit 7.3 compilation finds lambda parameter `created` shadowing `out GameObject created` at `Polity/KingdomPolityEndpointRuntime.cs:199`. Canonical gate uses its tracked `latest` language setting and does not expose this defect. Frozen runtime/Harness digests stayed unchanged; four exact inventories 2946/2950/2992/2996 and logs retained in `/tmp/taf-direct-qud-preflight.79T3BN`. Narrow rename remains queued; this failure is not a native gameplay result. |
...
| Tools tests | **PASS** — 316 / 316 tests | Repository tooling, eight actual-shell lifecycle tests with mocked Windows boundaries, eight process-ownership source contracts, generators, gallery census, documentation contracts, and package helpers are green at this checkpoint. |
