...
namespace ThousandAndFirst.Harness
{
	...
	public sealed class KingdomScenarioSaveProvider : IKingdomScenarioVerbProvider
	{
		internal const string Verb = "subsidence-save-check";
		internal const string Receipt = "r_TAF_ScenarioSubsidenceSave_v1";
		...
		{
			Ok = false;
			if (Verb != KingdomScenarioSaveProvider.Verb || !string.IsNullOrEmpty(Argument))
				return "subsidence-save-check takes no arguments";
			...
			string report = KingdomScenarioSaveChecks.Run(game, zone, out Ok);
			if (!KingdomScenarioDurableState.ProvesExactText(Receipt, "intent"))
			{
				Ok = false;
