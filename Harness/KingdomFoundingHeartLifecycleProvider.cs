...
namespace ThousandAndFirst.Harness
{
	...
	public sealed class KingdomFoundingHeartLifecycleProvider : IKingdomScenarioVerbProvider
	{
		internal const string Verb = "founding-heart-lifecycle";
		internal const string Receipt = "r_TAF_ScenarioFoundingHeartLifecycle_v1";
		internal const int ExpectedCases = 15;
		...
		{
			Ok = false;
			if (Verb != KingdomFoundingHeartLifecycleProvider.Verb || !string.IsNullOrEmpty(Argument))
				return "founding-heart-lifecycle takes no arguments";
			XRLGame game = The.Game;
			Zone zone = The.Player?.CurrentZone;
			if (!Eligible(game, zone, out string failure)) return "native founding heart lifecycle refused: " + failure;
			game.StringGameState.Add(Receipt, "intent");
			if (!KingdomScenarioDurableState.ProvesExactText(Receipt, "intent"))
				return "native founding heart lifecycle refused: test intent did not persist exactly";
			string report = KingdomFoundingHeartLifecycleChecks.Run(game, zone, out Ok);
