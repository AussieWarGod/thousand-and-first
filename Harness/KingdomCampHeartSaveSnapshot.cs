...
namespace ThousandAndFirst.Harness
{
	internal sealed class KingdomCampHeartSaveSnapshot
	{
		...
		internal readonly string StoreId, FireId, TimberId, ContentsDigest, BrushDigest;
		...
		internal KingdomCampHeartSaveSnapshot(string gameId, string realmId, string cityId,
			...
			string timberId, string contentsDigest, string brushDigest, int heartX, int heartY,
			int storeX, int storeY, int fireX, int fireY, int water, long turns)
		{
			...
			TimberId = timberId; ContentsDigest = contentsDigest; BrushDigest = brushDigest;
			HeartX = heartX; HeartY = heartY; StoreX = storeX; StoreY = storeY;
			FireX = fireX; FireY = fireY; Water = water; Turns = turns;
		}
	}
}
