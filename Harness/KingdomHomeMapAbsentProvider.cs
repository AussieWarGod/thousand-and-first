...
namespace ThousandAndFirst.Harness
{
	...
	public sealed class KingdomHomeMapAbsentProvider : IKingdomScenarioVerbProvider
	{
		internal const string Verb = "home-map-absent-save-witness";
		...
		internal static void RequireScript() => KingdomScenarioSaveFiles.Require(
			KingdomScenarioScript.TryRead(out IList<string> script, out _) && IsExact(script), "absent-home script differs");
		public string RunScenarioVerb(string name, string argument, out bool ok)
		{
			...
			try
			{
				...
				KingdomScenarioSaveFiles.Require(name == Verb && string.IsNullOrEmpty(argument)
					...
					"absent-home save context differs");
				string result = KingdomHomeMapAbsentWitness.Record(The.Game);
				ok = true; return result;
			}
			catch (Exception error)
			{
				KingdomLog.Log("absent-home retained failure: " + error);
				return "native-absent-home failed; " + KingdomScenarioRules.Bounded(error.Message);
			}
		}
	}
}
